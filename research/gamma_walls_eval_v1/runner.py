"""Study #22: dealer/gamma wall distances → Elastic Net logistic."""
...
def write_report(report, out_dir):
    ...
    path = out / (
        f"gamma_walls_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "gamma_walls_eval",
    )
    args = ap.parse_args()
    ...
    s = report["summary"]
    print(
        f"gamma_walls_eval_v1 — {s['verdict']} "
