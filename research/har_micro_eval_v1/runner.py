"""Study #29: HAR + microstructure joint → Elastic Net logistic."""
...
MICRO = ["spread", "flow_imbalance", "pin_score", "vwap_dist_pts", "hedging_flow_score"]
N_FEATS = 3 + len(MICRO)
...
def _load(db: Path, ticker: str, hz: str):
    ...
    for r in rows:
        ...
        if any(v is None for v in feats):
            continue
        try:
            mf = [float(v) for v in feats]
        except (TypeError, ValueError):
            continue
        if not all(np.isfinite(v) for v in mf):
            continue
        out.append((float(r[0]), str(r[1]), mf))
    return out
...
def write_report(report, out_dir):
    ...
    path = out / (
        f"har_micro_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "har_micro_eval",
    )
    args = ap.parse_args()
