"""Study #28: microstructure feature stack → Elastic Net logistic."""
...
def write_report(report, out_dir):
    ...
    path = out / f"micro_stack_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "micro_stack_eval",
    )
    args = ap.parse_args()
    ...
    s = report["summary"]
    print(
        f"micro_stack_eval_v1 — {s['verdict']} "
