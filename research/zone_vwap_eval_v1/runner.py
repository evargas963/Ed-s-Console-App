"""Study #25: zone / VWAP geometry → Elastic Net logistic."""
...
def write_report(report, out_dir):
    ...
    path = out / (
        f"zone_vwap_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "zone_vwap_eval",
    )
    args = ap.parse_args()
