"""Study #21: HAR × realized_vol / spread interactions → Elastic Net logistic."""
...
def _load(db: Path, ticker: str, hz: str):
    label = f"outcome_{hz}"
    q = (
        f"SELECT ts_utc, {label}, realized_vol, spread FROM snapshots_1m_normalized "
        f"WHERE ticker=? AND timeframe='1m' AND {label} IS NOT NULL "
        f"AND realized_vol IS NOT NULL AND spread IS NOT NULL"
    )
    c = sqlite3.connect(f"file:{db}?mode=ro", uri=True, timeout=30.0)
    rows = c.execute(q, (ticker,)).fetchall()
    c.close()
    return [(float(a), str(b), float(d), float(e)) for a, b, d, e in rows]
    ...
    for ticker in prereg["family"]["tickers"]:
        ...
        for hz in prereg["family"]["horizons"]:
            if hz in invalid_hz:
                continue
            key = f"{ticker}:{hz}"
            labeled = _load(Path(db_path), str(ticker), str(hz))
            xs, ys, dates = [], [], []
            for ts, y, rv, sp in labeled:
                j = bisect.bisect_right(ends, ts) - 1
                if j < 15:
                    continue
                h = feats[j]
                xs.append(np.concatenate([h, [rv, sp], h * rv, h * sp]))
                ys.append(y)
                dates.append(_et_date(ts))
            X = np.asarray(xs, dtype=np.float64) if xs else np.zeros((0, 11))  # caps-ok: zero-row design matrix of the correct width (no values invented); X.shape[0]==0 / no folds routes the cell to verdict UNDER_SAMPLED with mcc None
...
def write_report(report, out_dir):
    ...
    path = out / f"interaction_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "interaction_eval",
    )
    args = ap.parse_args()
    ...
    s = report["summary"]
    print(
        f"interaction_eval_v1 — {s['verdict']} "
