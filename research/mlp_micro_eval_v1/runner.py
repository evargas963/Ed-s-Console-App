...
def write_report(report, out_dir):
    ...
    path = out / f"mlp_micro_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "mlp_micro_eval",
    )
    args = ap.parse_args()
