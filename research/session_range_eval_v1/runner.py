"""Study #27: session range position → Elastic Net logistic."""
...
def _load_rows(db: Path, ticker: str, hz: str):
    ...
    label = f"outcome_{hz}"
    q = (
        f"SELECT ts_utc, {label}, spot, session_high, session_low, spread "
    ...
    c.close()
    xs, ys, dates = [], [], []
    for ts, y, spot, hi, lo, sp in rows:
        if any(v is None for v in (spot, hi, lo, sp)):
            continue
        try:
            spot_f, hi_f, lo_f, sp_f = float(spot), float(hi), float(lo), float(sp)
        except (TypeError, ValueError):
            continue
        if not all(np.isfinite(v) for v in (spot_f, hi_f, lo_f, sp_f)):
            continue
        width = hi_f - lo_f
        if width <= 1e-9:
            continue
        pos = (spot_f - lo_f) / width
        xs.append([pos, hi_f - spot_f, spot_f - lo_f, sp_f])
        ys.append(str(y))
...
def write_report(report, out_dir):
    ...
    path = out / (
        f"session_range_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
    )
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "session_range_eval",
    )
    args = ap.parse_args()
    ...
    s = report["summary"]
    print(
        f"session_range_eval_v1 — {s['verdict']} "
