"""Study #13: L1 book imbalance + spread + last_size → Elastic Net logistic."""
...
def _load_rows(db: Path, ticker: str, hz: str):
    ...
    for ts, y, bid, ask, spread, last in rows:
        ...
        if denom <= 0:
            continue
        imb = (bid_f - ask_f) / denom
        xs.append([imb, sp_f, last_f])
        ys.append(str(y))
...
def write_report(report, out_dir):
    ...
    path = out / f"l1_book_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "l1_book_eval",
    )
    args = ap.parse_args()
