"""Study #16: HAR features with per-regime logistic fits."""
...
def _load_labeled_regime(db: Path, ticker: str, hz: str):
    label = f"outcome_{hz}"
    q = (
        f"SELECT ts_utc, {label}, regime_primary FROM snapshots_1m_normalized "
    ...
    c.close()
    out = []
    for ts, y, reg in rows:
        if y is None:
            continue
        out.append((float(ts), str(y), str(reg or "UNKNOWN")))
    return out


def _fit_predict_regime(
    Xtr: np.ndarray,
    ytr: np.ndarray,
    reg_tr: np.ndarray,
    Xte: np.ndarray,
    ...
    # overlay per-regime models where eligible
    for reg in sorted(set(reg_tr.tolist())):
        mask_tr = reg_tr == reg
        if int(mask_tr.sum()) < MIN_REGIME:
            continue
        y_sub = ytr[mask_tr]
        ...
        idxs = np.where(mask_te)[0]
        for i, p in zip(idxs, p_sub):
            preds[int(i)] = p
    return preds
    ...
    for ticker in prereg["family"]["tickers"]:
        ...
        for hz in prereg["family"]["horizons"]:
            if hz in invalid_hz:
                continue
            key = f"{ticker}:{hz}"
            labeled = _load_labeled_regime(Path(db_path), str(ticker), str(hz))
            xs, ys, regs, dates = [], [], [], []
            for ts, y, reg in labeled:
            ...
            for train_days, test_days in folds:
                ...
                if tr.sum() < 50 or te.sum() < 1:
                    continue
                preds = _fit_predict_regime(
                    X[tr], y_arr[tr], reg_arr[tr], X[te], reg_arr[te], seed
                )
...
def write_report(report, out_dir):
    ...
    path = out / f"regime_har_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "regime_har_eval",
    )
    args = ap.parse_args()
    ...
    s = report["summary"]
    print(
        f"regime_har_eval_v1 — {s['verdict']} "
