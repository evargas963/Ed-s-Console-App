"""Study #26: spy_iwm_divergence + local geometry → Elastic Net logistic."""
...
FEATS = ["spy_iwm_divergence", "vwap_dist_pts", "net_delta"]
...
def write_report(report, out_dir):
    ...
    path = out / f"divergence_eval_{report['generated_utc'][:10]}_{report['run_id']}.json"
...
def main() -> int:
    ...
    ap.add_argument(
        "--out-dir",
        type=Path,
        default=Path(__file__).resolve().parents[2] / "reports" / "divergence_eval",
    )
    args = ap.parse_args()
    ...
    s = report["summary"]
    print(
        f"divergence_eval_v1 — {s['verdict']} "
