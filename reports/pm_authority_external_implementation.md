...
| 4 | external PM-authority reader wiring | **fail closed**: when executable PM authority is missing / unreadable / invalid, `pm_mission_edit_violation` returns `PM-FIRST BLOCK` for mission-gated paths and never falls back to the Git-tracked JSON. `measure_report()['pm_authority_mode']` signposts `enforcing`/`unavailable_fail_closed`. (`test_c_external_state_missing_fails_closed_and_blocks_mission_gated_product`) |
