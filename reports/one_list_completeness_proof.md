# One-list method-pivot evidence — 2026-08-22
...
python3 -c "from tools.find_it_fix_it_lock import second_work_list_violations, derive_active_obligations, second_authority_prose_violations, five_status_authority_violations, three_iteration_method_pivot_violations, ticker_specific_fix_scope_violations, master_closure_missing_root_cause; print(second_work_list_violations({'_check_second_list': True})); print(derive_active_obligations(open('governance/root_cause_log.md').read())); print(second_authority_prose_violations()); print(five_status_authority_violations()); print(three_iteration_method_pivot_violations({})); print(ticker_specific_fix_scope_violations())"
...
python3 -c "from tools.pretooluse_guard import master_admits_production_edit; print(master_admits_production_edit('server.py', current_text='- [x] \`OD-0\` — STATUS=PASS — (1) a (2) b (3) c (4) d (5) ROOT: x\\n', head_text='- [x] \`OD-0\` — STATUS=PASS — (1) a (2) b (3) c (4) d (5) ROOT: x\\n')); print(master_admits_production_edit('server.py', current_text='- [ ] \`OD-NEW\` — STATUS=NOT_PROVEN — universal defect\\n', head_text=''))"
python3 -c "import json; print(json.load(open('reports/one_list_reconciliation_counts.json')))"
```

CONTENT_HEAD = `81f27e23d00a7e662ec1856edf4469eb34ca60c0`
...
STATUS sum = 2371 ; equals MASTER_TOTAL = True
...
## Historical mapping (proven before freeze)
...
- Production edit + applicable unresolved master item → ALLOW (`test_negative_control_our_production_file_allowed_with_master_obligation`, `test_front_loaded_allows_when_master_admits`).
...
- New material master requirement → becomes the only admitted current obligation (`test_master_admits_new_unresolved_id_even_if_file_not_named`).
...
`python3 -m pytest tests/test_pretooluse_guard_repo_scope_v1.py tests/test_path_authority_v1.py tests/test_find_it_fix_it_lock_v1.py tests/test_five_why_recursive_lock_v1.py tests/test_five_why_reaches_bedrock_v1.py tests/test_five_why_recursion_lock_v1.py tests/test_rc_status_vocabulary_v1.py tests/test_enforced_check_negative_controls_v1.py tests/test_stop_guard_v1.py tests/test_log_law_v1.py tests/test_universal_ticker_scope_v1.py tests/test_requirement_proof_v1.py tests/test_ui_mockup_lock_v1.py -q` → 245 passed.
