...
`window.ED_RTH_END_MINS` from `static/rth_clock_authority.js` (generated from
`time_et.rth_clock_js_source`, rewritten at desk lifespan).
...
- `GET /static/rth_clock_authority.js` → `200`, body byte-equal to `time_et.rth_clock_js_source()` (`window.ED_RTH_START_MINS=570;\nwindow.ED_RTH_END_MINS=960;\n`).
