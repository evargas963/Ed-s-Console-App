...
from ml_horizon import default_training_label_column, live_inference_horizon_slug
...
def main():
    ...
    print("\n" + "-" * 72)
    print("3. LSTM FEATURE COMPLETENESS (FEATURES_5M + FEATURES_1M)")
    print("-" * 72)
    ...
    print("   (Uses same FEATURES_5M as LSTM — identical stats)")
    print("-" * 72)
    print("   See section 3 above — Transformer uses encode_snapshot_5m (1m snapshots) / FEATURES_5M.")

    print("\n" + "-" * 72)
    _live_hz = live_inference_horizon_slug()
    print(f"5. META-LEARNER READINESS (active per-ticker {_live_hz} models)")
    print("-" * 72)
    ...
    print(f"   models/active/SPY/xgb_SPY_{_live_hz}.pkl:       {'YES' if xgb_exists else 'NO'}")
    print(f"   models/active/SPY/lstm_SPY_{_live_hz}.pt:       {'YES' if lstm_exists else 'NO'}")
    print(f"   models/active/SPY/transformer_SPY_{_live_hz}.pt: {'YES' if trans_exists else 'NO'}")
    if not any([xgb_exists, lstm_exists, trans_exists]):
        print("   → Meta-learner cannot train yet (no base model files).")

    # ── 6. SEQUENCE AVAILABILITY (SPY) ───────────────────────────────────────
    print("\n" + "-" * 72)
    print("6. SEQUENCE AVAILABILITY (SPY)")
    print("-" * 72)
    ...
    print("\n" + "-" * 72)
    print("11. ACTIVE ARTIFACT COMPLIANCE (provenance)")
    print("-" * 72)
    try:
