...
async def request_stream_qos(stream, stats: "CaptureStats", level: int = STREAM_QOS_LEVEL) -> dict:
    ...
    try:
        ...
        content = ((reply.get("response") or [{}])[0].get("content") or {}) if isinstance(reply, dict) else {}
        outcome["code"] = content.get("code")
        outcome["msg"] = content.get("msg") or f"{type(e).__name__}: {str(e)[:200]}"
    stats.qos = outcome
