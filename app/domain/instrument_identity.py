...
RC-505 day 2: moved here from the repository root UNCHANGED — same constant, same function,
byte-for-byte the same logic. The root path `instrument_identity.py` is now a re-export shim,
so all 91 existing importers keep working and can be re-pointed a few at a time.
"""
