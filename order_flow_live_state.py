...
_receive_seq: dict[str, int] = {}  # symbol -> monotonic local receive sequence
...
def get_receive_log(symbol: str) -> list[dict]:
    """All L1 last-print receipts including restatements, in receive order."""
    sym = ticker_storage_key(symbol)
    if not sym:
        return []
    with _lock:
        log_q = _receive_log.get(sym)
        return [dict(x) for x in log_q] if log_q else []
...
def get_stats() -> dict[str, Any]:
    ...
    with _lock:
        return {
            ...
            "receive_log": {k: len(v) for k, v in _receive_log.items()},
            "top": list(_top.keys()),
        }
