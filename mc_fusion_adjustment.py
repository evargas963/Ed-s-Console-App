...
def normalize_mc(mc_output: Mapping[str, Any], spot_price: float) -> dict[str, float]:
    """
...
def fuse_payload_apply_mc_adjustment(fusion: Any, mc_out: Any, spot_price: Optional[float]) -> Any:
    ...
    pre = (
        float(getattr(fusion, "prob_up", 1 / 3)),
        float(getattr(fusion, "prob_down", 1 / 3)),
        float(getattr(fusion, "prob_flat", 1 / 3)),
    )
