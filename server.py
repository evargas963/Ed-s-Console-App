...
from math_exposure import (
    # RC-REHAB-1 (thirty-eighth slice): the MISSING_GREEK_SENTINEL / gamma_is_plausible /
    # bucket_metric pass-through re-exports are gone -- app/api/routes/debug.py and
    # terrain.py now import them from math_exposure directly.
    _f,
...
def _logger_fetch_and_log(ticker: str) -> str:
    ...
    try:
        ...
        _tq._note_terrain_success(tk)   # Cursor-audit F4: a clean fetch clears any streak/soft hold
        return "ok:fetch"
        ...
        if "vendor_status=" in _detail:
            ...
            if _val.isdigit():
                _vstatus = int(_val)
        _tq._note_terrain_failure(tk, err, _tq._classify_chain_failure(_vstatus, type(e).__name__))
        return f"error:{err}"
