...
# RC-REHAB-1 (2026-09-23, twenty-third slice): safe_get_price_history's top-level
# binding here had no real caller even before this slice -- its only other call site
# (the enrollment-seed path) already does its own local `from schwab_client import
# safe_get_price_history`, shadowing this one. Its sole real caller,
# _exposures_for_state, moved to server_state_exposures.py, which imports it
# directly; ruff -F401 caught the now-genuinely-dead top-level import.
...
from math_exposure import (
    ...
    # RC-REHAB-1 (twenty-first slice): bucket_metric's own call sites inside
    # _predictive_positioning_for_state moved to server_state_predictive_positioning.py
    # (which imports it directly), but app/api/routes/terrain.py:get_terrain_strikes
    # still does `from server import (..., bucket_metric, ...)` -- kept bound here as
    # that re-export surface, same pattern as MISSING_GREEK_SENTINEL/gamma_is_plausible.
    bucket_metric,  # noqa: F401
    _f,
    # RC-REHAB-1 (2026-09-23, twenty-third slice): compute_exposures_by_strike/
    # build_summary_rows/build_walls_rows/build_totals_rows/pick_net_gex_peak_strike's
    # only call sites moved with _exposures_for_state into server_state_exposures.py,
    # which imports all 5 directly.
    compute_gamma_void_zones, gamma_at_price,
    infer_strike_increment, required_strike_count,
    # RC-REHAB-1 (2026-09-23, thirty-third slice): gex_magnitude_label / gex_regime_label /
    # total_gamma_raw_at_strike / flow_imbalance_label_from_normalized's only callers moved
    # with the payload projection into server_state_payload.py.
...
# RC-REHAB-1 (2026-09-23, twenty-sixth slice): _derive_session's top-level binding here
# had no real caller even before this slice -- its only other call site already does its
...
# RC-REHAB-1 (2026-09-23, twenty-second slice): MarketVolContextV1/
...
# RC-REHAB-1 (2026-09-23): _schedule_analytics_recompute and the private cluster of
# helpers that exist only to serve it (_invalidate_analytics_cache_after_bg_failures,
# _reset_analytics_bg_fail_count, _record_analytics_bg_failure,
# _fetch_state_sse_bounded, _stamp_analytics_freshness_on_completed_fetch,
...
# RC-REHAB-1 (2026-09-22): REST Cum Delta accumulator state + its updater + its sole
# caller (_order_flow_data_for_state) moved to server_state_order_flow.py as one unit.
# _rest_cum_delta re-imported here (same dict object, mutations shared by reference) for
# _fetch_state's own direct read site further below.
from server_state_order_flow import _rest_cum_delta  # noqa: F401
...
from calibration.option_chain_morning_full import (
    ...
    # as that re-export surface, same pattern as MISSING_GREEK_SENTINEL/bucket_metric.
...
# _TRADER_UI_PRODUCT_HORIZONS moved to stack_runtime_governance.py along with its
# sole reader, _attach_stack_runtime_and_governance (RC-REHAB-1, twenty-eighth slice).
...
def _gamma_flip_and_void_zones_for_state(
    ...
    RC-569 (2026-09-21, live-RTH finding): `gamma_flip` here used to be an independent
    compute_gamma_flip_v2(contracts_use, ...) call -- contracts_use is the SELECTED-EXPIRY
    slice, the exact narrow-chain basis the RC-33/v23 terrain-SSOT migration already
    rejected for the live-UI-facing kl_gamma_flip field ("one terrain, one chain" -- see
    the "Terrain read: single source of truth" comment elsewhere in this file). That
    migration fixed the field consumers actually render but missed this one, which feeds
    ONLY the background snapshot logger (snapshots.gamma_inflection) -- so the live UI was
    never wrong, but every historical/ML snapshot row was computed by a second, narrower,
    already-rejected producer. REPRODUCED live: terrain reported gamma_flip=None for META
    (flip_diag: 'no_zero_crossing_regime_still_defined' -- gamma stays positive across the
    whole 560-940 examined window, a real absence, not a gap) while this function's own
    ...
    gamma_flip_conf = None
    gamma_flip_diag = _flip_snap.get("flip_diag") if _flip_snap else None
...
# RC-REHAB-1 (2026-09-22): order-flow-data assembly phase moved to
# server_state_order_flow.py, along with the REST Cum Delta accumulator it drives.
from server_state_order_flow import _order_flow_data_for_state  # noqa: F401
...
# alone drove.
from server_state_vol_envelope_sector import _vol_envelope_and_sector_for_state  # noqa: F401
...
from server_state_persistence_tail import _post_publish_persistence_tail  # noqa: F401  (RC-REHAB-1: moved to its own module, re-exported for tests/callers)
...
def _fetch_state(
    ...
    *,
    ...
    # ── Chain + quote in parallel ─────────────────────────────────────────────
    # RC-REHAB-1 (Phase 4, _fetch_state decomposition, thirty-fourth slice): pool choice,
    # chain gate, auth -> 401 and vendor-status -> 502 moved to server_state_intake.py.
    _cq = _fetch_chain_and_quote_for_state(
    ...
    # RC-REHAB-1 (Phase 4, _fetch_state decomposition, thirty-third slice): the ~575-line
    # payload projection moved to server_state_payload.py. Each phase hands over its own
    # NamedTuple instead of ~40 unpacked locals; key order is preserved.
...
# RC-REHAB-1 (2026-09-23): _per_strike_view_update_expiry moved to
# gamma_surface_eager_refresh.py along with its sole caller,
# refresh_gamma_surface_from_stream (twenty-seventh slice).
...
# RC-REHAB-1 (2026-09-23): _gamma_surface_contracts_with_stream_overlay moved to
# terrain_refresh.py along with its sole caller (twenty-fifth slice).
from terrain_refresh import _gamma_surface_contracts_with_stream_overlay  # noqa: F401
...
# refresh_gamma_surface_from_spot_tick (RC-570, symmetric siblings -- the per-tick
# eager-refresh hooks that keep Key Levels/the gamma heatmap live between REST
# cycles) moved to gamma_surface_eager_refresh.py, along with
# _per_strike_view_update_expiry (twenty-seventh slice). Both are passed as
# first-class callables at startup from _app_lifespan's own body, still in
# server.py, so this re-export stays.
from gamma_surface_eager_refresh import (  # noqa: F401
