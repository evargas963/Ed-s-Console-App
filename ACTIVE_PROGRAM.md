...
7. **Three-role loop** — this agent is coder, auditor, and PM. After code: audit (drift-audit + read-only subagent on material changes) → land on `main` when the claim is system-of-record → take the next PA-46 child. A PR is not done. Do not split to “Claude codes / I audit” as the standing method. Subagents are for a second-pass audit, not a second owner.
