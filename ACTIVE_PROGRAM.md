...
6. Console layout / "where the data sits" waits on **PA-36** and **RC-292**. The number is true before the furniture moves. Do not add KEY LEVELS tabs. Exposures, not raw greeks. Chart / Console / Terrain / Liquidity Map already split the same levels — F15 and LP-01 are the surface queue, not more cards.
...
4. **Console layout** — after PA-36 / RC-292. Six-pill UI lock stays (`tests/test_issue18_ui_contract.py`). Do not resurrect removed surfaces.
