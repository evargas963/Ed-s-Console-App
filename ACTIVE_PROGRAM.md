...
- [ ] **SIG-01 scoreboard/actionability accrual** — sessions 2–5 of segmented multi-day evidence toward signal-outcome validation; logger ~32 tickers, snapshot rows landing all session. (Folded 2026-09-06 from the retired closure schema: 2 of 5 RTH sessions accrued as of its last revision `0a49e6bc`; done when five are accrued and read back from the runtime.)
- [ ] **RUNTIME-SEPARATION move (operator host; RC-523 landed the code side 2026-09-06)** — with the desk stopped: move `data/` (the ~34 GB live DB + `-wal`/`-shm`, `data/barchart`), `logs/`, `schwab_token.json` and `diagnostics/` from the production checkout to `Trading\runtime\EdWebConsole\`; move the runtime-written `reports/` files (`terrain_backtest_latest.*`, `terrain_scorecard_history.jsonl`, `terrain_quarantine_ledger.jsonl`, `operable_surface_*_latest.json`, `fp_scoreboard_latest.json`) to `Trading\artifacts\EdWebConsole\reports\`; set `ED_RUNTIME_ROOT` and `ED_ARTIFACTS_ROOT` in the repo-root `.env` (see `docs/host/ENVIRONMENT_VARIABLES.md`); restart; probe `python -c "import runtime_layout as r; print(r.describe())"` → `separated: True` and `/api/build` healthy. Then launch every agent session from a worktree, never the production checkout. Done when that is proven for one full trading session; the follow-up (its own row) deletes the delegation, recovery door, live-path report and checkout rails, which exist only because production was a session cwd.
- [ ] **OPS-FULL-SUITE-STAMP** — operator host: fresh `npm run test:all` (E2E then pytest through the RC-535 output sink). Prior full run 2026-07-20: 4237 passed / 4 failed; the 3 code failures are fixed but the 4240/4241 tally is UNVERIFIED until a fresh complete run (Cursor veracity audit: "not re-proven"). (OPS-PLAYWRIGHT-E2E-RERUN left this file 2026-09-10 with the marker it existed to refresh — RC-542; required CI runs E2E on every PR and reports the exit code itself.)
...
| CR-02 | RETIRED 2026-09-23 (operator: no non-Schwab source; the Alpaca leg was removed from the capture daemon — IEX is ~5% of volume and nothing read it) | ~~**Trade prints + CVD (capture)**: Alpaca free IEX websocket (operator opens free account), 30-symbol prints, quote-rule signing, CVD; Schwab-signed vs IEX-signed correlation recorded ≥3 sessions.~~ |
