...
7. **Three-role loop** — see `AGENTS.md`. After code: audit → land on `main` when the claim is system-of-record → next row in Sequence above. A PR is not done. Subagents are a second-pass audit, not a second owner.
