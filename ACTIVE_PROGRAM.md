...
| TU-13 | QUEUED | **Volume-weighted regime input — parallel profile.** Born from the 2026-07-22 head-to-head (operator-forced): on 1,028 shared single-name days, volume-substituted gamma-at-spot predicts realized range at rho −0.2203 vs OI's −0.1612 (rule pre-set; `weighting_head_to_head` in the backtest report). Run volume-weighting as a PARALLEL scorecard profile (never silent swap — the TU-04 pattern), A/B daily vs OI, promote on sustained evidence; interacts with the single-name label restoration gate. |
