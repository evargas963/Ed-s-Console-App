...
- [x] **UI-CARD-PROVENANCE-CHIPS** @ `5e83315` — **superseded @ audit reaudit:** Cursor landed parallel Decision Rail surfaces (`dr-fusion-authority-strip`, `dr-empirical-context-line`) while ACTIVE_PROGRAM §UI card provenance is Claude design brief; Claude unified `signal-rail-card` + operator chip @ this SHA. Tests `tests/test_issue18_ui_contract.py` (25/25).

- [x] **QQQ-WEIGHTED-PUSH-HISTORICAL-NULLS** @ `5e83315` — `backfill_weighted_pushes` recomputes from constituent `chg_pct` + `confluence_quote_ticks` as-of `ts_utc` for panel symbols without snapshot columns (WMT, IWM holdings); IWM uses `blend_iwm_weighted_push` (55/45 holdings+sector) matching `iwm_blended_participation_push`; tests `test_qqq_weighted_push_full_top_matches_build_confluence`, `test_iwm_weighted_push_matches_blended_participation`, `test_backfill_weighted_pushes_uses_quote_ticks`.
...
- [ ] **AUDIT-CURSOR-DATA-UI-SLICE-2026-05-27** — **Claude full Read audit** of Cursor commits `f078593` … `6222cc6` on `feature/institutional-key-levels`. Handoff: `ACTIVE_PROGRAM.md` §Claude audit handoff. **Cursor reaudit fixes @ `5e83315`:** QQQ full-top + quote-tick backfill parity, IWM 55/45 blend parity, Claude UI cleanup committed, OPEN_ITEMS reconciled. **Closes when:** Claude reaudit sign-off @ `5e83315` with file:line citations.
