...
- [x] **Action 11.10 — bayesian_fusion.py FusionPayload + directional fabrications** — core in `1a68229`; tail: `_resolved_regime_label`, skip `evidence.get` neutral default, support attrs, `_model_dominant_class`, direct `posteriors[...]` emit. SHA: __________
...
- [x] **Action 11.12 — regime_engine.py zero-evidence primary + zone_since_bars** — return `_unknown_regime()` when `max(scores)<=0`; breakout fresh-zone skip when zone bars unknown. SHA: __________
...
- [ ] **Action 12.1+ — Layer 5 remaining surface** — `prediction_engine`, `multi_horizon_decision`, `liquidity_value_engine`, `ml_*`, `features/*`, full `server.py`/`market_state.py` re-read, `signals.py` L91-102 + ML `SimpleNamespace` fallback namespaces, `mc_fusion_adjustment._triplet` degenerate renorm.
