...
- [ ] **MEGA4-INVENTORY-REAUDIT-CALIBRATION-AND-REMAINDER** — follow-on multi-slice audit. After the arch_competition slice (this commit), ~455 mega4 ALLOWLISTED entries across the remaining subdirectories still carry the boilerplate justification and need source-verified reclassification. Same pattern: pure math / validator / builder / filesystem ops → NONE; genuine DB orchestrators → ALLOWLISTED with accurate justification. Subdirectories pending: `calibration/*` (largest batch), `bayesian_fusion.py`, `normalized_training_sync.py`, `ml_predict.py` (mega4 entries only), `train_all.py` mega4 entries, `verify_active_models.py`, top-level `monte_carlo*`, plus a long tail. Recommend slicing by subdirectory and committing each slice independently for review.
