...
- [ ] **DORMANT-TABLES-PRE-WIRE** `[REAL-GATE: unwalked-file]` — Tables with zero read consumers per `governance/artifacts/persistence_consumer_map.json`. **Current list (Pass 4 narrowed via audit-tool fix):** `confluence_log`, `news_events`. **Removed from list:** `level_crosses` (Pass 4 wired @ this row's LEVEL-CROSSES-WIRE sub-row), `session_log` (audit-tool fix: internal db.py readers now counted, reader=1), `logging_universe_eviction_log` (reader=1), `logging_universe_migration_log` (reader=1). **Why REAL-GATE not immediate fix:** each remaining table needs individual wire-or-drop decision (Pass 7 for confluence_log; news_events is a separate operator-product question — see audit-map row). **What this row gates:** Pass 1b pre-commit check `check_persistence_writer_has_reader` allows new INSERTs against these tables without blocking, because dormancy is tracked here. Removing a table name from this row before its wire-or-drop closure will cause Pass 1b to block any future INSERT touching it. **Closes when:** every named table is either wired with a reader landed in same commit, or dropped with schema + writer + migration in same commit.
