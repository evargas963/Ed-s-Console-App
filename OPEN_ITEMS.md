...
- **STATUS_CHANGE 2026-08-14 dual read-only audit @ `efe73f1`:** two independent audits agree. Z1 write-site + Z5 17-key payload stay accepted as instance repairs; do not regress. Class-prevention is NOT_PROVEN for Z1/Z3/Z4/Z5 (example-locked plants; high escape rates on renamed-equivalent variants). Z3 keep-the-split is accepted as the live-feature repair; `_FEATURE_PATH_POC` file+function exemption is a regression of RC-330's own class. Z2 gate is honestly scoped. Required checks at `efe73f1`: pytest-full run 31805215233 RED (`_stamp_gamma_pin_consumer_copy` uninventoried; vanna HTML needle; board-count snapshot pins); hardening run 31805212010 RED (ruff F401 unused `import re`). No new `[x]`. Do not merge. Parents stay OPEN.
- **STATUS_CHANGE 2026-08-14 CI fix-forward after `efe73f1`:** MEGA1 row for `_stamp_gamma_pin_consumer_copy`; vanna P1B guard re-pointed at `KEY_LEVEL_CONSUMER_REGISTRY`; unused `import re` dropped; board-count test keeps only `added == set()`. Charter slice lock (2) now names MEGA1_FILES / MEGA2_FILES + `test_mega1_traceable_audit.py`; slice lock (4) requires a STATUS_CHANGE that cites an audited SHA to cite required-check conclusion. No new `[x]`. Do not merge. Parents stay OPEN.
