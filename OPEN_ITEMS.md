...
- [ ] **Pilot 1 Schwab walk — deferred FINDs (2026-05-19, chunks 3-N)** — Disclosed in chunk-1 walks of `order_flow_engine.py` (`dfa1f82`) and `liquidity_value_engine.py` (`73bb17f`); not blocking but eligible for future paired-fix chunks. Cross-linked from `governance/PILOT_1_SCHWAB_WALK_AND_AUTHORITY_UX_STAGING.md` closure section.
  - [ ] FIND-OF3/OF4 — `_normalize(None)→0.0` silent neutral mass in `_compute_order_flow_score` (L768–791); fix is either renormalize-over-present-components or fail-closed composite when too few inputs.
  - [ ] FIND-OF5 — `_readiness(score, rvol)` uses `(rvol or 0) > 1.2` (L819) — fail-conservative direction is OK but unlabeled when rvol is None.
  - [ ] FIND-OF6/OF7 — `_direction(0.0)=="neutral"` and `math_exposure` ImportError → `"FLOW NEUTRAL"`/`"→"` (L805–811); semantic-label substitution, not Schwab-leaf fabrication.
  - [ ] FIND-LVE1 — `cluster_price_levels_into_zones` (L580–583) silently falls back from `"atr"` mode to `"percent"` when `atr_value is None`; no logger.info — operators can't detect ATR-mode-degraded clustering.
  - [ ] STYLE-LVE2 — inconsistent tag matching pattern (`in tags` vs `in str(tags)`) within same snapshot builders; standardize.
  - [ ] Magic-threshold disclosures — POC shift 0.002 (L894), VWAP-vs-POC 0.001 (L901/903), new_value_area 0.005 (L1029), zone-edge proximities 0.995/0.998/1.002 (L708/725/728).
