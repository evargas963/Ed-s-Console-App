...
- [x] **`d17.replaced_count` vs perf_proof (14 vs 12 drift)** — Root cause: disposition merge re-stamped two spurious `server.py:4478` rows (`return {}` surface / `pattern_kind_miss`) via cross-pattern `by_surface` + stable `by_id` on line-changed code. Fix: `cli.py` cross-pattern surface dedup `(path, surface)`; merge surface guard on `by_id`/`by_site`/`by_surface`; `cross_validate.covered_lines` counts all languages. Tests in `test_schwab_universal_coverage_scanner_v3.py`. Partial regen (`--max-files 400 --embedding-mode mock`): `server.py:4478` REPLACED rows **0**; `replaced_count_d17` 10 on this branch (4 perf_proof bundles). Operator branch: rerun scanner + `schwab_v4_scoreboard.py` to pin meta/scoreboard → expect **12/12** when 12 perf_proof links present. SHA: *(pending commit — scanner only; register gitignored)*.
