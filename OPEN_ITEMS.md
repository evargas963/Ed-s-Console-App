...
- [ ] **Pilot 1 Schwab walk — deferred FINDs (2026-05-19, chunks 3-N)** — Disclosed in chunk-1 walks of `order_flow_engine.py` (`dfa1f82`) and `liquidity_value_engine.py` (`73bb17f`); not blocking but eligible for future paired-fix chunks. Cross-linked from `governance/PILOT_1_SCHWAB_WALK_AND_AUTHORITY_UX_STAGING.md` closure section.
  ...
  - [x] FIND-OF6 — `_direction(0.0)=="neutral"` (L822–830): threshold-band design `[-0.15, 0.15]`, not fabrication; chunk-3 `_direction(None)→None` ensures only real scores label neutral.
  - [x] FIND-OF7 — `math_exposure` ImportError catch: pre-closed by 0edebc3 (Action 11.2) — verdict/labels/arrows `None`, agreement `unavailable` (not FLOW NEUTRAL / → / neutral).
