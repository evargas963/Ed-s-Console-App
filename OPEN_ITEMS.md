...
- **PROOF** is the minimum evidence class (`STATIC` < `ISOLATED_E2E` < `REAL_POPULATION` <
  ...
  Lower-class evidence is INVALID for the row, never partial credit; a record at a sha the
  code moved past, without runtime identity where required, or with a payload label standing
  in for provenance is INVALID (`governance.acceptance.evidence_status`; its falsifiers NC-13/
  14/15 are `tests/test_universal_closure_v1.py`, required CI — the executor does not certify
  itself).
...
|---|---|---|---|---|---|---|
| REQ-GOV-TRUSTED-VALIDATOR | FINITE | MERGE | delta:base_enforced_roster | STATIC | - | Every enforced check of the BASE roster judged the candidate tree running the BASE validator code (base tools overlaid on the candidate) and reports no more violations than on the base; no check the base enforces is removed without a base-side `retire:` row. The candidate's own validator is never executed by the trusted lane (a validator change is a trust-anchor change, authorized and reviewed by the operator). |
| REQ-GOV-TRUST-ANCHORS | FINITE | MERGE | tools.precommit_institutional:trust_anchor_paths | STATIC | - | Every file the enforcement path executes (hook wirings and their guards, the pre-commit config and its tools, every workflow, the institutional gate and its imports, the delta gate, the acceptance module, every SCOPE owner) is byte-identical to base, or its change is authorized by a base-side `anchor:` row. A base that cannot enumerate its own enforcement path is NOT_PROVEN; no smaller population stands in. |
...
| REQ-GOV-CLOSURE-COMMANDS | FINITE | MERGE | delta:closing_ledger_rows | STATIC | - | Every `governance/root_cause_log.md` row that becomes CLOSED or REMEDIATED in the delta cites at least one CI-executable command (interpreter-led: `python`/`pytest`/`node`/`npm`; a bare backticked file path is a mention) — judged as data by the trusted lane — and the required candidate lane (`hardening`, `tools/check_delta_adds_no_debt.py --base origin/main`) executed the first one in the candidate tree with exit 0. A regex match on a command string is not proof; a live-only probe (`curl`, `SELECT`) does not close a row by itself. |
