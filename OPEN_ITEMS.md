...
- [ ] **Action 11.1d — `compute_beta` R² residual** — `math_exposure_core.py:830` uses `r_sq = ... else 0.0` when ticker variance `< 1e-12`; reader cannot distinguish “no correlation” from “not computable”. Return `None` for `r_squared` in that branch (stats path, not direct Schwab trade-decision leaf).
...
- [ ] **Action 12.1 — prediction_engine.py fusion/empirical blend + narrative** — **CRITICAL** L223-225: `fusion_available=True` but `getattr(snap,"prob_*",1/3)` fabricates triplet when fusion attrs None (breaks post-12.0 contract); L1119-1122 narrative `dominant_outcome="unknown"`, `dominant_probability=0`, `model_agreement_label="low"`, `n_sources_active=0`; audit additional uniform/`confidence="low"` paths L151,L588,L609,L775. Schwab-leaf: per-horizon fusion snapshots from stack.
- [ ] **Action 12.2 — multi_horizon_decision.py probability fabrications** — L463-467 `_safe_prob` fallback `1/3`; L475-481 `_norm_triplet` uniform on degenerate sum; L504 `mins_to_close` default `180.0`; L581-583 canonical blend `getattr(...,1/3)` when stabilizing empirical-only horizons.
- [ ] **Action 12.3 — volatility_regime.py default policy fabrications** — L119 `vix_chg_abs = abs(vix_chg) if vix_chg is not None else 0.0`; L210-218 default `VolRegimePayload` emits trade_permissive=True with synthetic biases when classification inputs insufficient (fail-closed: unavailable payload vs fake neutral policy).
- [ ] **Action 12.4 — rules_engine.py zone_since_bars** — L181-182 `(zone_since_bars_1m or zone_since_bars) or 0` and `zone_since_bars_5m or 0` in alerts (mirror 11.12 regime_engine fix).
- [ ] **Action 12.5 — news_sentiment.py timeout/unavailable fabrications** — L309-317 snapshot flatten: `breaking_news_flag` 0 / empty headline when absent; L520-535 timeout path fabricates `breaking_news_flag=0`, `impact_level="LOW"`, empty sources (should distinguish unavailable vs low-impact).
- [ ] **Action 12.6+ — Layer 5 remaining unread surface** — `call_engine.py` full body; `micro_structure.py`; `liquidity_value_engine.py`; `ml_predict`/`ml_scheduler`/`ml_train`; `features/*` (11 files); `calibration/*`; `arch_competition/*`; `lstm_*`/`transformer_*`; `v2_decision/a2_option_expression.py`; `realized_contract_eval.py`; `training_cache.py`; re-read `server.py`/`market_state.py`; `signals.py` L91-102 + ML fallback namespaces; `mc_fusion_adjustment._triplet` degenerate renorm.
