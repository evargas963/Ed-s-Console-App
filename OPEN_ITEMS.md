...
- [ ] **F15** — POC/VAH/VAL — OPEN (Phase 2A / liquidity_value_engine)
  ...
  - [ ] Live path populates from the canonical producer — CLOSED-ON-OLD-LINE @ `45b28c33...` (#82), NOT IN LIVE TREE (RC-364): canonical tree computes `pl.today_poc` in `market_context.py` but `/api/state` does not stamp it and no `#dr-lvl-poc`/`#exec-poc` DOM binding exists in `static/`. PORT NEEDED: stamp + bind, old-line commit is the reference.
