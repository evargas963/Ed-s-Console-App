...
- [x] **Action 11.10 — bayesian_fusion.py FusionPayload + directional fabrications** — dataclass posteriors/probs/agreement default `None`; exception path `available=False` without fake triplet; `_model_direction_triplet`; no uniform 0.33 when no dir sources. **Residuals → 12.1:** `getattr(regime,"primary","unknown")`, `evidence.get(o,0.5)`, `getattr(*,"continuation_support",0.0)`, `dominant_class="flat"` defaults, `posteriors.get(...,0)` on happy-path emit. SHA: `1a68229` (landed as Action 12.0 batch 1)
...
- [ ] **Action 11.12 — regime_engine.py zero-evidence primary + zone_since_bars** — `max(scores)` with all-zero evidence; `zone_since_bars_1m or 0` fabrications in scoring helpers. SHA: —
...
- [ ] **Action 12.1+ — Layer 5 remaining surface** — bayesian_fusion residuals (11.10 tail), regime_engine (11.12), `prediction_engine`, `multi_horizon_decision`, `liquidity_value_engine`, `ml_*`, `features/*`, full `server.py`/`market_state.py` re-read, `signals.py` L91-102 + ML `SimpleNamespace` fallback namespaces.
