...
- [ ] **F15** — POC/VAH/VAL — OPEN (Phase 2A / liquidity_value_engine). **2026-08-14 fourth audit:** engine POC had no live consumer. Decision (a): display metric — carry engine POC/VAH/VAL on `/api/state` and render `#dr-lvl-poc` / `#exec-poc` (#82). Not (b) retrain, not (c) delete. Display consumer ≠ model consumer. RC-330 stays `[ ]` — fusion still uses the close-price 12-bin proxy (780.25 ≠ engine 780.75); no retrain table. Live-path child stays `[ ]` until #82 is an ancestor of `origin/main`. Remaining: one population site, replay/backfill, fallback, universality, runtime.
  ...
  - [ ] Live path populates from the canonical producer — Decision (a) on `cursor/f15-engine-poc-consumer-556d` @ `45b28c3` (tip `a9fb51e` withhold-window). `/api/state` stamps `today_poc`/`pd_poc` from `price_levels`; `#dr-lvl-poc` / `#exec-poc` bind `d.today_poc`. Not in the seven `[x]`. `[x]` only when `45b28c3` is an ancestor of `origin/main`.
...
- [ ] **RC-282** — Freshness / stale actionability — OPEN
  ...
  - [ ] Root code fix landed
  - [ ] Runtime proof on loaded code
- [ ] **RC-285** — model published `LIVE, edge=0` fabricated zero — OPEN (parent stays open). **2026-08-14 adversarial REJECT:** `1117f19` forbade fabricating edge as 0 then permitted fabricating it as `val_accuracy·100`. Write-site `None` stands. Accuracy fallback removed; UI renders `edge === null` as `—`. Fix/Proof + two consumer children `[x]` @ `1e09445259b1c0b1392cccdaa7b5b26d922af8d0` on `origin/main`. LSTM accuracy-as-edge is **RC-291** (OPEN; relabel on #79, not on `origin/main`). Universality stays `[ ]`.
...
- [ ] **RC-329** — one-producer gate blind to consumer-name→semantic — OPEN (parent stays open). **2026-08-14 second audit:** 1/17 was not one source. `KEY_LEVEL_CONSUMER_REGISTRY` now drives all 17 `kl_*` labels/tips in the payload; KEY LEVELS tables have no hardcoded `label:`. Fix `[x]` @ `1e09445259b1c0b1392cccdaa7b5b26d922af8d0` on `origin/main`. **Pin-fix rewrite (OPEN until on `origin/main`):** do not rebase or land `cursor/pin-fix-net-gex-label-556d` / `ef5c0a2` (339-commit paint). Rewrite is `cursor/pin-fix-registry-556d` from `46513e3`: no painted `kl_*` `label:`; `kl_gamma_pin` = `Net Γ Peak` from the registry. Proof stays `[ ]` until that rewrite SHA is an ancestor of `origin/main`.
...
- RC-285 → canonical RC-285 row (OPEN; Fix/Proof + two consumer children `[x]` @ `1e09445259b1c0b1392cccdaa7b5b26d922af8d0`; LSTM accuracy-as-edge is RC-291; universality remains)
...
- RC-329 → canonical RC-329 row (OPEN; Fix `[x]` @ `1e09445259b1c0b1392cccdaa7b5b26d922af8d0`; Proof stays `[ ]` — pin-fix rewrite not on `origin/main`)
...
| RC-285 write-site fabricated zero | `1117f19` / `1e09445259b1c0b1392cccdaa7b5b26d922af8d0` | three children `[x]` (semantic/live/root); Fix/Proof + two consumer children `[x]` @ `1e09445259b1c0b1392cccdaa7b5b26d922af8d0`; LSTM accuracy-as-edge is RC-291 |
| RC-301 except-literal sites + gate | `5d68d93` | five children `[x]`; CLASS / parent / RC-318 stay `[ ]` |
| F15 one POC math producer | `462a581` | two children `[x]` (semantic + mutation); live path / no-alternate / RC-330 stay `[ ]` |
...
| RC-329 `kl_gamma_pin` consumer bind | `bb85651` / `1e09445259b1c0b1392cccdaa7b5b26d922af8d0` | three children `[x]` (semantic/live/root); Fix `[x]` @ `1e09445259b1c0b1392cccdaa7b5b26d922af8d0`; Proof stays `[ ]` (pin-fix rewrite) |
