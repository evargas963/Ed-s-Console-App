...
- [ ] **DORMANT-TABLES-PRE-WIRE** `[REAL-GATE: unwalked-file]` — Six tables identified by `tools/audit_persistence_consumers.py` have writer methods but zero read consumers as of Pass 2 baseline `c868a62`: `level_crosses`, `confluence_log`, `session_log`, `news_events`, `logging_universe_eviction_log`, `logging_universe_migration_log`. **Why REAL-GATE not immediate fix:** each requires individual wire-or-drop decision (Passes 4/5/6/7 for the original four; news_events + logging_universe_* are separate operator-product questions). **What this row gates:** Pass 1b pre-commit check `check_persistence_writer_has_reader` allows new INSERTs against these tables without blocking, because dormancy is tracked here. Removing a table name from this row before its wire-or-drop closure will cause Pass 1b to block any future INSERT touching it — that is the intended forcing function. **Closes when:** every named table is either (a) wired with a reader landed in same commit, or (b) dropped with schema + writer + migration in same commit. Each closure removes its table name from this row and adds a `- [x] @ <SHA>` sub-row.
