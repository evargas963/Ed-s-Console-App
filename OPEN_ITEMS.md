...
- [ ] **LIVE-UI-A — Canonical 1/3 triplet consumer audit**  
  `canonical_forward_probs_for_display` (signals.py ~L64–72) withholds max-entropy display for one consumer; `canonical_forecast_from_fusion` (~L106–148) may still expose `probability_*` with `provenance="fusion_unavailable"`. **Risk:** any card reading `canonical.probability_up` without provenance shows fake 0.333. **Fix direction:** enumerate all canonical triplet consumers (via path-only Glob); single display helper or provenance gate everywhere.
  **Progress (multi-slice):**
  - **`call_engine._validate_trade` L2b @ `beeb16e`** — explicit `canonical_provenance_is_tradable(canonical.provenance)` gate (was implicit via `confidence="low"` side-effect of `canonical_forecast_from_fusion`). Paired tests in [`tests/test_action11_9_call_engine_fail_closed.py`](tests/test_action11_9_call_engine_fail_closed.py): `test_canonical_oppose_gate_blocked_by_non_tradable_provenance_even_when_confidence_elevated` + regression `test_canonical_oppose_gate_fires_for_tradable_bayesian_fusion_provenance`.
  - **`market_state.build_market_state` L1541-1561 @ `3313e1f`** — explicit `canonical_provenance_is_tradable(_cf.provenance)` gate on the dominant_prob stamp path; non-tradable canonicals now set `ms.dominant_prob = None` instead of the prior unconditional `round(_cf.dominant_probability(), 4)` = `0.3333` placeholder leak. `dominant_dir` left untouched (producer convention sets it to "flat" for non-tradable — fail-closed visible). Paired tests in [`tests/test_action12_7_market_state_fail_closed.py`](tests/test_action12_7_market_state_fail_closed.py): `test_dominant_prob_withheld_for_non_tradable_canonical_provenance` + AST-level lock `test_market_state_source_imports_canonical_provenance_gate`.
  - **JS-side regression locks @ `3313e1f`** — `test_live_ui_a_no_canonical_probability_reads_in_js` + `test_live_ui_a_no_dominant_prob_renders_in_js` in [`tests/test_live_ui_integrity_v1.py`](tests/test_live_ui_integrity_v1.py). Empirical enumeration (path-only, line-number-metadata only) at tip: zero current JS consumers of `canonical.probability_up/down/flat`, `.canonical_forecast.probability_*`, `d.dominant_prob`, `dominantProb` in `static/index.html`. server.py likewise does not stamp these fields into Tier C payload (canonical_forecast appears only in a comment at L1972). Tests fire if a future card binding adds a fake-0.333 surface.
  - **JS tradability gates @ `bc5705d`** — `isCanonicalTradable(d)` helper; `effectiveDirection` withholds when non-tradable; per-horizon Move P + Fused Confidence gated on `fusionActive` (`isFusionAuthoritative(d)`). Paired locks in [`tests/test_stack_wire_4_cand_ui_fusion_gate.py`](tests/test_stack_wire_4_cand_ui_fusion_gate.py); Playwright contract updated in [`tests/e2e/stack-wire-4-cand-ui-fusion-gate.spec.js`](tests/e2e/stack-wire-4-cand-ui-fusion-gate.spec.js) (split-brain `canonical_forecast_missing` → `null`, not `dominant_dir` fallback).
  - **Enumeration @ tip (path-only):** 3 Python reader sites of `.probability_up/down/flat`: (a) [`signal_types.CanonicalForecast.dominant_probability()` L186-192](signal_types.py:186) returns placeholder unconditionally — still leaks internally; (b) [`prediction_engine._forward_probs_from_canonical` L605-611](prediction_engine.py:605) already gated; (c) `call_engine._validate_trade` L1213+ now gated. Single producer of placeholder: `signals.canonical_forecast_from_fusion` (intended fail-closed semantic).
  - **Remaining (Python-internal):** `signal_types.CanonicalForecast.dominant_probability()` returning placeholder for non-tradable contaminates any future caller. Fix requires `Optional[float]` signature change + caller updates across (per path enumeration): `bayesian_fusion.py`, `multi_horizon_decision.py`, `v2_decision/module_a_adapter.py`, `v2_decision/a1_isotonic_calibration_attachment.py`, `v2_decision/a1_raw_probability.py`. Separate slice — call sites need individual Read to determine each consumer's withhold semantic (some may want default placeholder, some withhold).
