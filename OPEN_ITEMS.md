...
- [ ] **GOV-REMOTE-ENFORCEMENT** — the credential boundary. MEASURED 2026-09-10 (`gh api repos/evargas963/Ed-s-Console-App/branches/main/protection`): `enforce_admins=true`; required checks `pytest-full` + `hardening` (strict, matched by NAME, app_id 15368 = GitHub Actions); `required_approving_review_count=0`; no CODEOWNERS; no rulesets; the account that operates CI and merges is the account coding agents use. Consequence: no repository mechanism can prove WHO changed an enforcement file or merged a PR, and both required checks run CANDIDATE code — the repository cannot defend the checker's predicates against its own author (RC-539; the "trusted" lane that tried was deleted, RC-546). ATTACKED 2026-09-11 on 1a2eaafe (RC-539 row): the coding agent's credential IS the admin (`evargas963`, scopes `repo` + `workflow`, sole collaborator); scratch PR #240 (weakened predicate) and #241 (required step replaced by `true`) both obtained the required `hardening` status, so a candidate can change its own judge and workflow and still merge. AUTHORITY GRAPH 2026-09-11 (RC-539 row): author, pusher and merger are one principal; a limited token of the same account is the same principal (scratch PR #242: `Can not approve your own pull request`), and the admin credentials (`gh:github.com:evargas963`, `git:https://github.com`) sit in this host's credential store where every agent process can read them. The minimum trust root is therefore a SECOND principal, and every other control follows from it. Met when, in this order, each step attacked and refused: (1) a second GitHub account for coding agents (e.g. `evargas963-agent`), added as a collaborator with `write` only — never admin/maintain; its token a fine-grained PAT with Contents write, Pull requests write, Metadata read, NO Workflows, NO Administration; that account's `gh auth login` on the agent host, and the operator's `gh:github.com:evargas963` and `git:https://github.com` entries REMOVED from the agent host's credential store (`cmdkey /list` shows none); (2) `.github/CODEOWNERS` on `main` with `@evargas963` owning: `/.github/` (CODEOWNERS and both workflows), `/.pre-commit-config.yaml`, `/.claude/settings.json`, `/.cursor/hooks.json`, `/tools/hook_chain.py`, `/tools/operator_law_guard.py`, `/tools/process_lock_guard.py`, `/tools/stop_guard.py`, `/tools/operating_process_lock.py`, `/tools/shell_parse.py`, `/tools/pretooluse_guard.py`, the merge-path import closure `/tools/check_delta_adds_no_debt.py`, `/tools/check_institutional_correctness.py`, `/tools/mission_latch.py`, `/tools/check_market_correctness.py`, `/tools/check_absence_has_a_type.py`, `/tools/check_one_producer.py`, `/tools/check_single_stream_authority.py`, `/tools/check_test_claims_are_executed.py`, `/tools/check_credential_leak.py`, `/tools/check_private_paths.py`, `/tools/check_eol_style_invariant.py`, `/tools/check_schwab_market_field_semantics.py`, `/tools/check_ui_data_integration.py`, `/tools/check_vendor_field_coercion.py`, `/tools/data_faucet_audit.py`, `/tools/find_prove_locks.py`, `/tools/phase2a_level_lock.py`, `/tools/universal_scope_lock.py`, `/db_authority.py`, `/runtime_layout.py` (the checker imports these two application modules for the faucet audit's DB path), and the registries the checks read as authority: `/governance/computation_registry.json`, `/governance/level_faucets.json`, `/config/decision_path_admissions.json`; (3) branch protection on `main`: `required_approving_review_count` 1, `require_code_owner_reviews` true, `dismiss_stale_reviews` true, `require_last_push_approval` true, `enforce_admins` true, required checks `pytest-full` + `hardening` strict from app 15368, force pushes and deletions off (whether count 0 + code-owner reviews alone gates only owned paths is NOT_PROVEN; count 1 is the proven form and the operator's approval replaces the chat merge word). RE-ATTACK that must then FAIL for the agent account: push a workflow edit (refused at push: no `workflow` permission); open PRs that weaken `tools/check_institutional_correctness.py`, rubber-stamp the gate, retire a check, neutralise `tools/check_credential_leak.py`, edit `governance/computation_registry.json`, delete `.github/CODEOWNERS` — each must show "Review required" from `@evargas963` that the agent cannot supply; `gh api -X PUT .../branches/main/protection` and `gh pr merge --admin` as the agent must return 403/404; `cmdkey /list` on the agent host must show no operator entry. Nothing in this repository can install or prove any of it; all three steps are the operator's. (GOV-ROOT-LEDGER-SCAN, a design-only mechanism with no measured failure, was removed 2026-09-10 under the immune rule.)
...
- **RC-297** (derivation inventory drift) — ledger row closed; the board accepts it on the standing lock `tests/test_provenance_v1.py` (RC-532: the population locks — every served route classified, every MarketState field categorised, every engine argument listed, every producer_ref a real function and a real row — replaced `test_mega2_traceable_audit.py`'s every-function coverage).
