...
- [x] **GOV-REMOTE-ENFORCEMENT** — DESCOPED 2026-09-11 (operator: the wrong threat model). MEASURED 2026-09-10/11 (`gh api repos/evargas963/Ed-s-Console-App/branches/main/protection`, still true — `enforce_admins=true`, required checks `pytest-full` + `hardening`, `required_approving_review_count=0`, no reviewer-identity file, no rulesets): required CI runs candidate code in the candidate's own tree, so it is regression detection for an honest change, not a security boundary against a repository administrator who deliberately rewrites the checker, the gate or the workflow — three scratch PRs proved exactly that (#240, #241; a reviewer-identity file was excluded per operator instruction and never installed, see RC-475 below) before this row was closed. **Containing a hostile repository administrator is out of scope** — coding agents are development agents: they inspect, edit, test, commit, push and open/update PRs normally, and CI catching a real regression plus the operator's own MERGE word is the whole authority model, unchanged from AGENTS.md's existing law (no bot accounts, no per-file authority machinery, no second-principal architecture). What is real and stays true, stated once and not reopened as a program: required CI cannot stop a credential that also administers the repository from weakening its own judge; that is a property of who holds the credential, not a defect a commit can fix. RC-539 carries the historical measurement.
