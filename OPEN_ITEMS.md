...
- [ ] **RC-329** — one-producer gate blind to consumer-name→semantic — OPEN (parent stays open). **2026-08-14 second audit:** 1/17 was not one source. `KEY_LEVEL_CONSUMER_REGISTRY` now drives all 17 `kl_*` labels/tips in the payload; KEY LEVELS tables have no hardcoded `label:`. **Pin-fix rewrite (OPEN until on `origin/main`):** do not rebase or land `cursor/pin-fix-net-gex-label-556d` / `ef5c0a2` (339-commit paint; `merge-tree --write-tree 46513e3` conflicts on `static/index.html` and re-paints `label:`). Rewrite is `cursor/pin-fix-registry-556d` from `46513e3`: no painted `kl_*` `label:`; `kl_gamma_pin` = `Net Γ Peak` from the registry. Proof stays `[ ]` until that rewrite SHA is an ancestor of `origin/main`.
