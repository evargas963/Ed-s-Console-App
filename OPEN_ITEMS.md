...
- [ ] **RC-292** — Gamma-pin semantic collision — OPEN (parent stays open). **2026-08-13/14 measured on this `main`:** Console `kl_gamma_pin` is stamped from `cs.gamma_pin` (`server.py`) = `pick_gamma_pin_strike` = largest `|net GEX$|` per 1% on the selected expiry. HVL is the total-gamma concentration. `pick_pin_and_strength` / `pick_net_gex_peak_strike` / `chart.html` are absent on this `main`. **Label/tooltip/lock @ `0e304f6`. `pin_score` @ `6d14ee2`. Persist + semantic stamp @ `d71bb5e`.** Remaining: universality, runtime proof. Charm no longer emits a colliding `gamma_pin` alias.
  ...
  - [x] Backward-safe migration for persisted values — Closed @ `d71bb5e`. `gamma_pin_semantic=net_gex_peak`; NULL on old rows means the same writer semantic; numbers not rewritten. Tests: `tests/test_institutional_key_levels.py`.
...
- RC-292 → canonical RC-292 row (OPEN; label/tooltip/lock @ `0e304f6`; pin_score @ `6d14ee2`; persist/migration @ `d71bb5e`)
...
| `[x]` on this board | 12 | 2026-08-13 STATUS_CHANGE rows (UI-01, PHASE-4, UI-04 P1B/P1C/P1D, ML-META) plus six RC-292 children (label / tooltip / lock @ `0e304f6`; pin_score @ `6d14ee2`; persist / migration @ `d71bb5e`). Parent RC-292 stays `[ ]`. |
...
| RC-292 persist + migration | `d71bb5e` | two children `[x]`; parent stays `[ ]` |
