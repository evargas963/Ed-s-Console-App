...
- [x] **Action 12.0 — Layer 5 upstream fail-closed (batch 1)** — `monte_carlo.py` `mc_feature_dict`, `bayesian_fusion.py` FusionPayload defaults + directional triplet, `mc_fusion_adjustment.py` normalize_mc, `signals.py` canonical_forecast + fusion stage + MC regime passthrough, `call_engine.py` 11.9b residuals. SHA: `1a68229`
- [ ] **Action 12.1+ — Layer 5 remaining surface** — `prediction_engine`, `multi_horizon_decision`, `liquidity_value_engine`, `ml_predict`/`ml_scheduler`, `features/*`, `regime_engine` classify path, full `server.py`/`market_state.py` re-read, `signals.py` L91-102 + ML fallback namespaces.
