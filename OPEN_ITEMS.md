...
- [ ] **STACK-WIRE-6a** — FIND-WIRE6-1..2 @ __________ — replay hold bars single authority: `RTH_SESSION_MINUTES = RTH_END_MINS - RTH_OPEN_MINS` derived in `time_et.py` (FIND-WIRE6-1, 390 magic removed from `replay_max_hold_bars_from_context`); `TRADE_TYPE_HOLD_BARS` dict + `MICRO_REGIME_HOLD_BARS_COMPRESSION` shared between `for_setup` + `for_trade_type` paths — `trade_type="none"` returns 0 in BOTH (FIND-WIRE6-2 parity break fix; was 20 in fallback vs 0 in live); tests `tests/test_stack_wire_6_v1.py` (5) + `tests/test_replay_hold_bars.py` parity case extended. **Gate:** STACK-WIRE-6b independent (parallel-safe); STACK-WIRE-6 parent closes when 6b + live/replay parity validation also land.
- [ ] **STACK-WIRE-6b** — magic-constant sweep in `realized_contract_eval.py` (`OPTION_MULTIPLIER`, strike-tolerance literals at 7 sites, JSON-validity `> 10` heuristic, ring-buffer retention `[-5000:]`/`[-200:]`) + `live_decision_bundle.py` env-tunable spot-refresh defaults (`ED_TICK_REFRESH_SPOT_PCT=0.0003` / `_SPOT_ABS=0.05`). FIND-WIRE6-3..7. Independent of 6a; parent STACK-WIRE-6 still open.
- [ ] **STACK-WIRE-6** — **Live vs replay parity** — parent item **LIVE-UI-F** — measured test live `ms_dict` vs reconstructed snapshot row (`v2_advisory_backfill.ms_dict_from_snapshot_row` produces fields that match live `ms_dict`). Closes when 6a + 6b also land.
