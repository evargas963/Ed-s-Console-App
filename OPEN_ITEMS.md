...
- [ ] **ML-META-JSON-VERIFICATION-ASYMMETRY** — CLOSED-ON-OLD-LINE @ `a107412` (PR #55), NOT IN LIVE TREE (RC-364): `ml_predict.py` on the canonical tree has no `lstm_meta` pre-deserialization verification. PORT NEEDED: verify `lstm_*_meta.json` in `_load_lstm` before `lstm_model.load_lstm` reads it, same pattern as `transformer_meta` — old-line commit is the reference.
...
- [ ] **RC-285** — model published `LIVE, edge=0` fabricated zero (semantic; live path; root cause; fix; proof; universality) — OPEN / NOT_PROVEN DETAILS
  ...
  - [ ] Unmeasured `edge_pp` is not published as `val_accuracy·100` — STILL OPEN ON LIVE (RC-364): `server.py:9298` registers LSTM with `edge_key="val_accuracy"` and `server.py:9271` publishes `raw*100` in the `edge` field. This is the RC-291 defect alive on the canonical tree — see RC-291 port item.
  - [ ] Model-health UI renders `edge === null` as `—` (not 0 / NaN / throw) — CLOSED-ON-OLD-LINE, NOT TREE-VERIFIED (RC-364): `formatModelHealthEdge` absent from live; verify live's model-health render path for the null case and close on live evidence.
- [ ] **RC-291** — LSTM reports `val_accuracy` as "edge" — CLOSED-ON-OLD-LINE @ `d9ddc8ac...` (#79), DEFECT ALIVE ON LIVE TREE (RC-364, tree-verified 2026-08-16): `server.py:9298` registers LSTM with `edge_key="val_accuracy"` and `server.py:9271` publishes `raw*100` in the `edge` field — accuracy masquerading as edge on the canonical app. PORT NEEDED (top of the RC-364 port list): relabel per the old-line reference — LSTM `edge_key` = `edge_pp` (None until a true edge exists); print `val_accuracy` under its own name; operator never sees accuracy in the edge slot.
  - [ ] LSTM `edge_key` is `edge_pp`, not `val_accuracy` — open on live (`server.py:9298`).
  - [ ] LSTM row label prints `val_accuracy` under its own name — open on live.
  - [ ] DOM/test: operator never sees accuracy in the edge slot — open on live.
