...
- [ ] **Action 11.6 — `server.py` vol_oi / iv_model_spread label defaults** — `server.py:5042,5049` still `.get("label", "unknown")` for `vol_oi_label` and `iv_model_spread_label`; drop default so snapshots persist NULL when upstream helpers withhold label.
