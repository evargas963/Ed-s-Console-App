...
- [ ] **STACK-WIRE-4** — FIND-WIRE4-1..4 @ __________ — verification slice (incl. v2_decision): NON_TRADABLE docstring diagnostic-only; `is_ms_dict_fusion_authoritative` leave-as-is; governed_stack_contract 5 constants; map `stack_mode` 4 UI sites; tests `test_stack_wire_4_v1.py` (5) + `test_fusion_contract` extension (9). **Gate:** STACK-WIRE-5 (independent).
