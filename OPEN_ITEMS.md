...
- [~] **FIND-SNAPSHOT-BAR-STAMP-V1** (write path LANDED RC-470; historical residual remains) — `EdDB.insert_snapshot` now floors `ts_utc` to the containing 1m bar start and rebuilds the ET clock from that instant (`horizon_outcomes.snapshot_bar_start_ts_utc`). New writes join `price_bars_1m` exactly. **Residual:** historical poll-second rows still need `BACKFILL_JOIN_TOL_SEC=29`; do not drop the production default until those rows are rebuilt. Connects to **FIND-LABEL-INTEGRITY-FORENSICS** (`TIMESTAMP_IDENTITY_NOT_PROVEN`).
