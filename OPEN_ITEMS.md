...
- [ ] **RC-329** — one-producer gate blind to consumer-name→semantic — OPEN (parent stays open). **2026-08-14 second audit:** 1/17 was not one source. `KEY_LEVEL_CONSUMER_REGISTRY` now drives all 17 `kl_*` labels/tips in the payload; KEY LEVELS tables have no hardcoded `label:`. Pin-fix lineage must still rebase onto main before land. `[x]` only on `origin/main` SHA.
