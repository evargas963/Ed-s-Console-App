...
- [ ] **GOV-REMOTE-ENFORCEMENT** — the credential boundary. MEASURED 2026-09-10 (`gh api repos/evargas963/Ed-s-Console-App/branches/main/protection`): `enforce_admins=true`; required checks `pytest-full` + `hardening` (strict, matched by NAME, app_id 15368 = GitHub Actions); `required_approving_review_count=0`; no CODEOWNERS; no rulesets; the account that operates CI and merges is the account coding agents use. Consequence: no repository mechanism can prove WHO changed an enforcement file or merged a PR, and both required checks run CANDIDATE code — the repository cannot defend the checker's predicates against its own author (RC-539; the "trusted" lane that tried was deleted, RC-546). ATTACKED 2026-09-11 on 1a2eaafe (RC-539 row): the coding agent's credential IS the admin (`evargas963`, scopes `repo` + `workflow`, sole collaborator); scratch PR #240 (weakened predicate) and #241 (required step replaced by `true`) both obtained the required `hardening` status, so a candidate can change its own judge and workflow and still merge. Met when (1) coding agents operate GitHub with a separate identity or fine-grained token that lacks `administration`, `environments`/`secrets`, bypass rights AND the `workflow` scope (GitHub refuses a push touching `.github/workflows` without it), and an attempt to alter protection, push a workflow change or merge around the required checks with it is refused — attacked, not configured; (2) a CODEOWNERS file names an identity the coding agent does not hold for `tools/check_institutional_correctness.py`, `tools/check_delta_adds_no_debt.py`, `tools/*_guard.py`, `tools/hook_chain.py`, `.github/workflows/`, `.pre-commit-config.yaml` and `.claude/settings.json` / `.cursor/hooks.json`, with `require_code_owner_reviews` and at least one required review on `main`, so those files cannot merge on the author's own approval. Nothing in this repository can install or prove either half; both are the operator's. (GOV-ROOT-LEDGER-SCAN, a design-only mechanism with no measured failure, was removed 2026-09-10 under the immune rule.)
