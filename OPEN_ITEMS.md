...
- [x] **Action 11.7 — hedging-flow charm normalization** — `server.py` passes `charm_normalized=None` when `_charm_net` is None so `compute_hedging_flow_score` partial-renorms without fabricating 0. SHA: __________
- [ ] **Action 12.0 — Layer 5 upstream fail-closed (batch 1)** — `monte_carlo.py` `mc_feature_dict`, `bayesian_fusion.py` FusionPayload defaults + directional triplet, `mc_fusion_adjustment.py` normalize_mc, `signals.py` canonical_forecast + fusion stage + MC regime passthrough, `call_engine.py` 11.9b residuals. Remaining queue: `prediction_engine`, `multi_horizon_decision`, `liquidity_value_engine`, `ml_*`, `features/*`, full `server.py`/`market_state.py` re-read.
