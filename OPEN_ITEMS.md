...
- [ ] **RC-329** — one-producer gate blind to consumer-name→semantic — OPEN (parent stays open). **2026-08-14 second audit:** 1/17 was not one source. `KEY_LEVEL_CONSUMER_REGISTRY` now drives all 17 `kl_*` labels/tips in the payload; KEY LEVELS tables have no hardcoded `label:`. **Pin-fix merge dry-run BLOCKER:** `git merge --no-commit origin/cursor/pin-fix-net-gex-label-556d` onto `origin/main`+this branch conflicts on `static/index.html` (and 24 other files). Semantic on that lineage is now `Net Γ Peak`; a clean line-7618 merge requires rebasing the paint branch onto this SHA. Do not land pin-fix until that rebase. `[x]` only on `origin/main` SHA.
...
- **STATUS_CHANGE 2026-08-14 defect-learning class plants:** uncited-instance plants now fail for hardcoded `kl_*` label, undelegated `*volume_profile*`, measurement-`*_key` literal fallback, uninventoried `*_engine.py`, and except-literal `-> float`. Charter sentence in `AGENTS.md`. No new `[x]`. Parents stay OPEN.
