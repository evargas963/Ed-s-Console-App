...
- [ ] **UI-01 analytics key identity** — server-resolved `selected_exp` (generation-guarded) + single client key-builder + `analytics_cache_key` payload echo, retiring the frozen-cards divergence
...
- [ ] **UI-04 P1B — vanna honesty** — the value shown as "vanna" is a vega/(S·iv) proxy: either relabeled as that proxy or replaced with true vanna
- [ ] **UI-04 P1C — charm sign gate** — charm analytic sign proven before it feeds the call-engine Greeks vote, or the vote gated until the sign is proven
