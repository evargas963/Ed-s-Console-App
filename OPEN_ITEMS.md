...
- [ ] **LIVE-UI-A — Canonical 1/3 triplet consumer audit**  
  ...
  **Partial @ pending-SHA:** `call_engine._validate_trade` L2b canonical-opposes branch now gates explicitly on `canonical_provenance_is_tradable(canonical.provenance)` (was implicit via `confidence="low"` side-effect of `canonical_forecast_from_fusion`). Defensive lock — current producer behavior is safe by coincidence; gate prevents future producer change or debug fixture from leaking placeholder 1/3-each triplets into the opposes-call veto. Paired tests in `tests/test_action11_9_call_engine_fail_closed.py`: `test_canonical_oppose_gate_blocked_by_non_tradable_provenance_even_when_confidence_elevated` + regression `test_canonical_oppose_gate_fires_for_tradable_bayesian_fusion_provenance`. **Enumeration @ tip (path-only):** 3 production consumers of `.probability_up/down/flat`: (a) `signal_types.CanonicalForecast.dominant_probability()` L186-192 returns placeholder unconditionally; (b) `prediction_engine._forward_probs_from_canonical` L605-611 already gated; (c) `call_engine._validate_trade` L1213+ now gated this commit. **Remaining:** `signal_types.dominant_probability()` needs `Optional[float]` return + ~6 caller updates (bayesian_fusion.py, multi_horizon_decision.py, v2_decision/module_a_adapter.py + others — full caller list pending source-Read); Tier C payload (server.py) + `static/index.html` UI surface not walked this slice.

- [ ] **LIVE-UI-B — Degraded stack visibility**  
  `stack_integrity_v1` events in `ml_bundle` / `PredictiveCard.stack_integrity_v1` (signals.py degradation sites; signal_types.py ~L283). **Risk:** `authority_intact=False` while UI looks healthy. **Fix direction:** operator-visible degraded badge when `stack_integrity_events` non-empty or `degraded=True`. **Partial @ pending-SHA:** `#dr-stack-integrity-degraded-chip` + `_updateStackIntegrityDegradedChip` when `stack_integrity_v1.degraded`; tests `tests/test_live_ui_integrity_v1.py`, `tests/test_stack_wire_3_ui_phase3_closure.py`.
