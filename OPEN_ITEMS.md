...
| REQ-GOV-CLOSURE-COMMANDS | FINITE | MERGE | delta:closing_ledger_rows | STATIC | - | Every `governance/root_cause_log.md` row that becomes CLOSED or REMEDIATED in the delta cites at least one CI-executable command (`python`/`pytest`/`node`/`tools/`) — judged as data by the trusted lane — and the required candidate lane (`hardening`, `tools/check_delta_adds_no_debt.py --base origin/main`) executed the first one in the candidate tree with exit 0. A regex match on a command string is not proof; a live-only probe (`curl`, `SELECT`) does not close a row by itself. |
