...
- [ ] **Action 11.9 — call_engine.py fail-closed on missing index quotes + fusion posteriors** — 11 high-priority sites + 5 lower-priority deferred; fusion posterior gate semantic: **block** trade when posterior is None (fail-closed). Schwab-leaf paths: `quotes.{SPY,QQQ,IWM}.netChange`, chain delta, fusion engine output.
- [ ] **Action 11.9b — call_engine.py lower-priority fail-open** — `compression_bars`, `zone_since_bars_5m`, `n_models_active` guarded default, readiness_score `.get(..., 0)`.
