...
- [ ] **GOV-REMOTE-ENFORCEMENT** — the credential boundary. MEASURED 2026-09-10 (`gh api repos/evargas963/Ed-s-Console-App/branches/main/protection`): `enforce_admins=true`; required checks `pytest-full` + `hardening` (strict, matched by NAME, app_id 15368 = GitHub Actions); `required_approving_review_count=0`; no CODEOWNERS; no rulesets; the account that operates CI and merges is the account coding agents use. Consequence: no repository mechanism can prove WHO changed an enforcement file or merged a PR, and both required checks run CANDIDATE code — the repository cannot defend the checker's predicates against its own author (RC-539; the "trusted" lane that tried was deleted, RC-546). Met when (1) coding agents operate GitHub with a fine-grained token that lacks `administration`, `environments`/`secrets` and bypass rights, and an attempt to alter protection or merge around the required checks with it is refused; (2) every PR that changes `tools/check_institutional_correctness.py`, a hook guard or a workflow carries the operator's review before merge. (GOV-ROOT-LEDGER-SCAN, a design-only mechanism with no measured failure, was removed 2026-09-10 under the immune rule.)
