...
- [ ] **MEGA4-INVENTORY-REAUDIT-CALIBRATION-AND-REMAINDER** — multi-slice audit, in flight. Source-verified slices landed so far (each commit reclassifies a per-function-audited file group): calibration/ batch 1 [`313194c`] (41 entries: json_utils, paths, canonical_1m_grid, db_guard, trust, statistical_integrity, signal_engineering); calibration/ batch 2 [`6f15431`] (85: a1_conformal_artifact_production, a1_isotonic_artifact_production, v2_a1_conformal, phase65_cleanup_v1, v2_a1_ev_bounds, v2_a1_execution_ev, v2_live_logging, movement_target_eval_common_v1, movement_target_phase5_discrimination_v1, movement_target_phase65_cleanup_v1, movement_target_phase65_isolation_v1, movement_target_phase6_edge_v1); top-level batch [`c870055`] (30: ml_predict, train_all, normalized_training_sync); analyze_phase batch [`fe63d71`] (11: analyze_phase3 + partial analyze_phase4); anchor/edge batch [`e74afa5`] (42: anchor_audit, audit_phase1, canonical_enforcement, edge_discovery, edge_validation, eval_movement_targets_phase_style_v1); schema/writer/discrim batch [`acf6866`] (19: schema, writer, legacy_report, validate_logging_e2e, validate_outcome_join, signal_layer_discrimination + analyze_phase4 _inc same-pattern cleanup). **Total source-reviewed: ~258 entries across ~32 files.** **Still pending (boilerplate-ALLOWLISTED count at HEAD acf6866 = 216):** calibration/{backfill_outcomes, backfill_signal_layer_v1_bundle, build_trusted_anchor_proof_dataset, canonical_1m_grid_scan, movement_target_phase6_edge_v1, phase65_edge_isolation_v1, phase6_edge_discovery_governed_v1, repair_anchor_coverage_pad_v1, repair_canonical_1m_*, run_production_accumulation_validation, v2_a1_calibration, v2_advisory_backfill, payload_audit, eval_movement_target_phase65_v1, eval_movement_target_phase6_v1}, lstm_data, lstm_model, ml_data_common, ml_horizon, ml_scheduler, ml_train, smoke_predict_active, train_compare, training_cache, training_provenance, transformer_model, transformer_train, xgboost_model, bayesian_fusion, monte_carlo*, verify_active_models, arch_competition/scheduler_integration leftover. Continuation pattern: per-file source read, NONE vs tightened-ALLOWLISTED, throwaway helper script per slice, delete-after-run, commit per file-group.
- [ ] **STACK-WIRE-5-CAND-OF-RESIDUAL-MAGICS** — follow-on (big-audit catch): name OrderFlowEngine residual bare integers — `_compute_book_imbalance(data, 1/3/5)` depths at `order_flow_engine.py:908-910` + `_compute_institutional_flow_proxy(data, 5)` at L772 (depths should be named `OF_BOOK_DEPTH_TOP/SHALLOW/DEEP` or similar); RVOL center `(rvol - 1.0)` at L843 (semantic "RVOL=1 is average" — name `OF_RVOL_NEUTRAL_CENTER`); `_weighted_mean_present` default `min_present=2` at L802 (callers should pass explicit, default should be the constant). Hygiene-tier; no Schwab leaves involved.
- [ ] **STACK-VERIFY-CAND-LOAD-TICKERS-RETURN-TYPE** — follow-on (big-audit catch of f832652 partial fix): `scheduler_user_tickers.load_user_scheduler_tickers` returns `list[str]` with silent empty-fallback on DB failure. Architectural fix: return `Optional[list[str]]` (None on DB unavailable) + update 6 callers (`lstm_data.py:679,892`, `ml_scheduler.py:153`, `train_all.py:59`, `transformer_train.py:167`, `verify_active_models.py:49`, `tests/test_issue22_logging_universe.py:220` mock). Deserves dedicated slice with caller-by-caller test coverage.
- [ ] **STACK-VERIFY-CAND-SILENT-FALLBACK-SWEEP** — follow-on (big-audit catch): pre-existing bare `except Exception:` blocks across multiple sweep3 TARGET_FILES (`market_context.py:823` `: return None`, `schwab_client.py:412-423` nested-except, etc.). Different from sweep3's `except Exception: pass` pattern but related silent-degradation anti-pattern. Broader sweep candidate.
