...
- [x] **Action 12.1b — prediction_engine.py residuals** — `_pack_horizon_row` uses `_tri_probs` (no `0.33`/`0.34`); DB-missing card + `probs_5c is None` → `None` empirical fields; enrichment narrative guards incomplete triplets. SHA: __________
- [x] **Action 12.2b — multi_horizon `_infer_trade_mode` fail-closed** — `mins_to_close` missing/invalid → `None`; synthesis `mode="unknown"`; `_primary_order_for_mode` intraday default stack without fabricating mins. SHA: __________
- [ ] **Action 12.7+ — Layer 5 remaining unread surface** — `call_engine.py` full body; `ml_predict`/`ml_scheduler`/`ml_train`; `features/*` (11 files); `calibration/*`; `arch_competition/*`; `lstm_*`/`transformer_*`; `v2_decision/a2_option_expression.py`; `realized_contract_eval.py`; `training_cache.py`; re-read `server.py`/`market_state.py`; `signals.py` L91-102 + ML fallback namespaces; `mc_fusion_adjustment._triplet` degenerate renorm.
