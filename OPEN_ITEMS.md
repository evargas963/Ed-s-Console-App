...
- [ ] **STACK-WIRE-5** — FIND-WIRE5-1..3 @ __________ — `order_flow_live_state.is_rth_open` 4th `RTH_OPEN_MINS` consumer (FIND-WIRE5-1); OF composite 22 named constants (Phase 6 ablation surface) — weights sum 1.00, clip / rvol-term / direction / readiness ladders + tape windows + norm divisors (FIND-WIRE5-2); OF freshness vs Tier C independence — 2 map rows for order-flow strip + stack vote layer (FIND-WIRE5-3); tests `tests/test_stack_wire_5_v1.py` (5/5). **Gate:** STACK-WIRE-6.
