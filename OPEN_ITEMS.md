...
- [x] **Action 11.3b — ms_dict EM/iv_skew/level_density/compliance + helpers** — 9 residual `.get` sites + `compute_em_progress` / `compute_iv_skew` / `compute_level_density` / `compute_iv_model_spread` / `compute_volume_oi_ratio` fail-closed when inputs absent. SHA: __________
- [ ] **Action 11.5 — `compute_net_charm` when contracts_used==0** — `math_exposure_core.py:735-747` still emits `charm_direction="neutral"` for zero net with no contracts; server pre-init is None (11.3) but helper should return direction/magnitude None when `used==0`.
- [x] **Action 11.6 — `server.py` vol_oi / iv_model_spread label defaults** — bundled in 11.3b with helper fail-closed (`compute_volume_oi_ratio`, `compute_iv_model_spread`). SHA: __________
- [ ] **Action 11.7 — hedging-flow charm normalization** — `server.py:3683-3686` passes `charm_normalized=0` when `_charm_net` is None (should pass `None` into `compute_hedging_flow_score` so partial composite excludes charm, per 11.4 renorm contract).
