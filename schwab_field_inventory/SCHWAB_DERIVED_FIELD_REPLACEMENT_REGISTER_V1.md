...
| CAPS silent-default family | DFR-009/011/018 repo-wide (full family) | CLOSED | `cab3ef4` — `tools/anti_pattern_sweep.py` + `test_anti_pattern_family_repo_wide.py`; prefix allowlist RETIRED 2026-09-23 (RC-REHAB-1), every hit reviewed per line |
...
## CAPS silent-default substitution family — no allowlist

Gate: `tools/anti_pattern_sweep.py`, enforced by `tests/test_anti_pattern_family_repo_wide.py`
and `tests/test_caps_marker_is_line_scoped_v1.py`. Scope: every git-tracked `.py` file,
including `tools/`, `tests/` and `governance/`. The only escape is a line-specific
`# caps-ok: <reason>` marker on the hit line.

RC-REHAB-1 (2026-09-23): the allowlist that used to be mirrored here (106 whole-file /
whole-folder `CAPS_PREFIX_ALLOWLIST` rows plus line-number `CAPS_LINE_ALLOWLIST` pins) is
deleted. It hid 1,967 hits across 393 files that had never been reviewed line by line. Every
hit was reviewed: real masked defects were fixed at the source, and each legitimate default
now carries its own reason in the code. The per-hit verdict ledger is in the commit that
retired the allowlist.
