...
|---|---|---|
| No competing heavy waves | MECHANICALLY_ENFORCEABLE_EXISTING_OWNER | ONE authorization path (RC-519). `tools/operating_process_lock.py` owns an atomic two-file lease in the host temp directory: `reserved.json`, created O_EXCL by `tools/process_lock_guard.py` at the command (a bounded 45 s reservation), and `held.json`, claimed O_EXCL by the admitted process and bound to its pid + create time. `admit_heavy_launch` runs in-process at every launch seam — the pytest controller (`tests/conftest.py`), `tools/check_delta_adds_no_debt.py`, `tools/check_institutional_correctness.py`, `tests/institutional_e2e_boundary_campaign.py` — so a `-c` payload, `cmd /c`, `pwsh -Command`, a subprocess or any other form still passes it; a child of an admitted wave passes through the ancestor rule; two processes started in one instant are ordered by create time. A reservation older than its TTL or a lease whose pid is dead is reclaimed by the next decision; a filesystem error refuses. The command seam reads the ONE interpreter in `tools/operator_law_guard.py` (segments, wrapper options, payloads) and judges Python payloads by AST (`pytest.main`, `runpy`, literal `subprocess` argv) — a payload whose launch arguments are not literal is not judged at the command seam and is caught at the launch seam. The former ~2 s start-up window is closed: the interval between decision and visibility is owned by the reservation |
