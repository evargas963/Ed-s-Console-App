...
**Writer:** the AI the **operator selects** to run (Architecture A). `governance/sole_writer.json` is a **non-authoritative tombstone** — no persisted writer/auditor/vendor field selects or vetoes the working AI. Authority is not a value stored anywhere in the tree (RC-461): the operator selects the working AI via `ED_AGENT_ROLE`, and authority CHANGES require operator approval at merge (CODEOWNERS + branch protection over the authority files), enforced locally by `tools/writer_drift_lock.control_authority_violation`. There is no off-repo authority file, privileged helper or host provisioning - that design was removed as overbuilt. **Auditor:** Cursor (falsify after landings).
