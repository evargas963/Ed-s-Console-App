...
| Card cluster: Position sizing strip | `r_units` | `signal_types.TheCall` (default `None`) → `market_state.py:1433` → `server.py:4569` / `5057` | SSE Tier C | `decision_generation_id` | `None` when sizing omitted; explicit `0.0` from `compute_position_size` NO_TRADE is sized zero-R (not withheld) | `tests/test_stack_wire_1_v1.py::test_r_units_none_propagates_end_to_end` | FIND-WIRE1-1 + FIND-WIRE1-1-CALLCARD; **Phase 3:** `STACK-WIRE-3-UI-R-UNITS-NONE` |
