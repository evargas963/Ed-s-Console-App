...
|---------|-------|----------|-------------|--------------|------------|------|---------------|-------------|
| Decision Command rail | `stack_runtime.stack_mode` | `server.py:1993` (`classify_stack_health` in `_attach_stack_runtime_and_governance`; single call site per `tests/test_stack_wire_4_v1.py`) | SSE Tier C (`/api/analytics/state`, `/api/stream`) | `decision_generation_id` + `_server_build_ts` | Enum only: FULL / PARTIAL / DEGRADED / INVALID; never inject non-authority values. **UI consumers (4 sites):** `#dr-stack-mode-chip` + `stack_runtime` reads in `static/index.html` ~L3780, ~L4119, ~L6369, INVALID title ~L6436. `signals_engine_failed` sibling ~L5362-5365 (no static consumer — Phase 3 `STACK-WIRE-3-UI-SIGNALS-ENGINE-FAILED-BADGE`). **v2_decision:** `is_ms_dict_fusion_authoritative` in `a1_raw_probability.py` + `module_a_adapter.py` | `tests/test_audit_cand_server_py_full_read_v1.py::test_stack_mode_value_is_authority_only`, `tests/test_stack_wire_4_v1.py` | FIND-SERVERPY-15 + STACK-WIRE-4 | producer-only closed @ 05c48d8 |
...
| Internal | `chains.callExpDateMap.*.multiplier` / `chains.putExpDateMap.*.multiplier` | Schwab leaf; consumer `realized_contract_eval._contract_multiplier(ct)` — read per chain row in `_contract_pnl_at_horizon` + `_evaluate_realized_contract_trades_for_rows` (entry_ct multiplier path) | SSE Tier C / archived `option_chain_json` | n/a | Fail-closed on missing leaf — new "missing_multiplier" skip-reason in trade log; no hardcoded 100 fallback (per SCHWAB FULL REPO directive) | `tests/test_stack_wire_6b_v1.py::test_contract_multiplier_reads_schwab_leaf_fail_closed` | FIND-WIRE6-3 |
