...
MEGA2_TRACEABLE_INVENTORY: tuple[Mega2TraceableDerivation, ...] = (
    ...
    Mega2TraceableDerivation("order_flow_engine.py", 278, "_latest_content_field", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "PR214 Gap 1: the ONE freshness-aware per-field resolver over Schwab LEVELONE_OPTIONS/EQUITIES partial/delta content items (_latest_content_field)."),
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 113, "_read_daemon_upstream_health", "ALLOWLISTED", None, (), 'mega1_diagnostic_log', "FRESHNESS/HEALTH SEMANTIC AUDIT 2026-08-30: reads the canonical daemon's own status file (stream_spine.HealthRegistry via tools/run_stream_capture.py's write_status) for the REAL per-service Schwab-socket truth, distinct from this module's local-replay-proxy streaming_healthy (_read_daemon_upstream_health)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 140, "_log_stream", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_log_stream)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 148, "_streaming_healthy", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_streaming_healthy)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 159, "is_order_flow_stream_running", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (is_order_flow_stream_running)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 163, "get_plane_authority_for_ticker", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_plane_authority_for_ticker)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 180, "streaming_l1_cache_usable", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Fast-quote gate: plane row fresh within FAST_QUOTE_STREAM_CACHE_MAX_AGE_MS (streaming_l1_cache_usable)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 200, "_stream_db_identity_status", "ALLOWLISTED", None, (), 'mega1_diagnostic_log', "PR214 Gap 2: producer identity via the shared data plane -- reads the daemon's stream_producer_heartbeat row through this process's own resolved stream_capture.db connection, proving identity structurally rather than by comparing two independently-resolved path strings (_stream_db_identity_status)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 490, "begin_option_contract_command", "NONE", None, (), None, "No market-field derivation: PR214 premerge gap 2 -- issues a monotonic generation number so a superseded option-subscription command cannot overwrite newer desired state (begin_option_contract_command)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 577, "_read_producer_option_contracts", "ALLOWLISTED", None, (), 'mega1_diagnostic_log', "PR214 premerge gap 1A: PRODUCER-side option subscription identity -- the current OPEN coverage epoch symbol per Schwab option service, read from the canonical stream_capture.db, distinct from the server's DESIRED/requested contract (_read_producer_option_contracts)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 190, "get_streaming_diagnostics", "ALLOWLISTED", None, (), 'mega1_diagnostic_log', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_streaming_diagnostics)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 245, "_identity_forces_unhealthy", "NONE", None, (), None, "No market-field derivation: pure fail-closed gating over an already-computed stream_db_identity verdict plus the existing subscribe-grace window (_identity_forces_unhealthy)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 214, "_open_capture_db_readonly", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "SINGLE-STREAM-AUTHORITY repair 2026-08-30: read-only capture-DB handle for the live-plane feed, replacing the retired second StreamClient (_open_capture_db_readonly)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 230, "_replay_new_rows", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Replays daemon-captured L1/book rows into order_flow_live_state/live_market_plane, the same plane-ingest calls the retired direct-socket handlers made (_replay_new_rows)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 271, "_replay_option_contract_rows", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Same replay shape as _replay_new_rows for the one active option contract's LEVELONE_OPTIONS/OPTIONS_BOOK rows (_replay_option_contract_rows)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 309, "_feed_loop", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Poll loop replacing the retired StreamClient message loop; opens zero Schwab connections (_feed_loop)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 452, "_contract_matches_underlying", "NONE", None, (), None, "No market-field derivation: exact vendor OSI-root identity of an already-selected vendor contract symbol against the active underlying."),
    Mega2TraceableDerivation("order_flow_streaming.py", 556, "clear_active_option_contract", "NONE", None, (), None, "No market-field derivation: drops desired option-contract state and writes an empty daemon signal so a foreign contract cannot survive an underlying switch (clear_active_option_contract)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 461, "_ensure_default_option_contract_for_ticker", "NONE", None, (), None, "No market-field derivation: requests the server-owned default vendor contract; pick lives in app.options.contracts.default."),
    Mega2TraceableDerivation("order_flow_streaming.py", 356, "set_streaming_active_ticker", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (set_streaming_active_ticker)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 379, "set_active_option_contract", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Requests LEVELONE_OPTIONS/OPTIONS_BOOK for one option contract via the daemon signal file (set_active_option_contract)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 404, "get_option_contract_book_microstructure", "DERIVED", None, ("order_flow_engine.py:compute_book_microstructure",), None, "Order-flow semantic product for one option contract's live book — delegates to the SAME producer the equity route reads, never a second book-imbalance computation (get_option_contract_book_microstructure)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 419, "_option_streaming_healthy", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "FRESHNESS/HEALTH 2026-08-30: same feed-connection health gate as _streaming_healthy, mirrored for the independent option-contract slot (_option_streaming_healthy)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 431, "get_option_contract_streaming_diagnostics", "ALLOWLISTED", None, (), 'mega1_diagnostic_log', "FRESHNESS/HEALTH 2026-08-30: same diagnostics shape as get_streaming_diagnostics, mirrored for the independent option-contract slot (get_option_contract_streaming_diagnostics)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 467, "start_order_flow_stream", "DERIVED", None, ("order_flow_live_state.py:get_content_for_symbol",), None, "Delegates to Schwab transport producers for start_order_flow_stream."),
    Mega2TraceableDerivation("order_flow_streaming.py", 494, "stop_order_flow_stream", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (stop_order_flow_stream)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 511, "get_stream_thread", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_stream_thread)."),
