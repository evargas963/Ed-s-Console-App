...
MEGA2_TRACEABLE_INVENTORY: tuple[Mega2TraceableDerivation, ...] = (
    ...
    Mega2TraceableDerivation("math_volatility.py", 107, "vix_bucket", "DERIVED", None, ("server.py:_fetch_state", "market_state.py:build_market_state",), None, "VIX level buckets from quote input."),
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 97, "_stale_bucket", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_stale_bucket)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 112, "_diag_on_active_l1_tick", "ALLOWLISTED", None, (), "mega1_l1_sse_counters", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_diag_on_active_l1_tick)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 128, "_async_staleness_watch", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_async_staleness_watch)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 160, "get_streaming_diagnostics", "ALLOWLISTED", None, (), "mega1_diagnostic_log", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_streaming_diagnostics)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 180, "_resubscribe_to_ticker", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_resubscribe_to_ticker)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 208, "_resubscribe_coro", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_resubscribe_coro)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 221, "set_streaming_active_ticker", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (set_streaming_active_ticker)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 249, "_graceful_disconnect_stream_client", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_graceful_disconnect_stream_client)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 268, "_drain_asyncio_tasks", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_drain_asyncio_tasks)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 279, "_message_loop_until_shutdown", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_message_loop_until_shutdown)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 308, "_run_stream_loop", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_run_stream_loop)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 319, "_run_stream_loop._async_run", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_run_stream_loop._async_run)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 341, "_run_stream_loop._async_run._book_handler", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_run_stream_loop._async_run._book_handler)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 354, "_run_stream_loop._async_run._level_one_handler", "ALLOWLISTED", None, (), "mega2_schwab_stream_l1", "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_run_stream_loop._async_run._level_one_handler)."),
