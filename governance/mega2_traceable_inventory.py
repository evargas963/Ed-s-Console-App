...
MEGA2_TRACEABLE_INVENTORY: tuple[Mega2TraceableDerivation, ...] = (
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 68, "options_streaming_enabled", "NONE", None, (), None, "No market-field derivation: reads the ED_OPTIONS_STREAM gate at call time so enabling/disabling options collection is honoured without an import-time cache."),
    Mega2TraceableDerivation("options_stream_collect.py", 92, "register_options_handlers", "NONE", None, (), None, "No market-field derivation: attaches LEVELONE_OPTIONS/OPTIONS_BOOK handlers to the daemon's EXISTING StreamClient. Registration is inert until something subscribes, and failures are contained so the equity path cannot be aborted."),
    Mega2TraceableDerivation("options_stream_collect.py", 467, "start_options_collection", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Reconciles orphaned coverage, starts the bounded ingest writer, subscribes the two options services on the daemon's sole client, and runs the rotation. Reads no Schwab market field itself; selection rides stored chain data."),
    Mega2TraceableDerivation("options_stream_collect.py", 531, "stop_options_collection", "NONE", None, (), None, "No market-field derivation: closes coverage epochs and drains the writer at shutdown, so retained history never claims observability past the end of the session."),
    Mega2TraceableDerivation("options_stream_collect.py", 572, "options_stream_status", "NONE", None, (), None, "No market-field derivation: reports options collection enablement, subscribed contract count and ingest counters for the diagnostics surface."),
    Mega2TraceableDerivation("options_stream_collect.py", 74, "_options_frame_handler", "NONE", None, (), None, "No market-field derivation: builds the per-service options frame handler. Nested factory; the handler it returns owns the behaviour."),
    Mega2TraceableDerivation("options_stream_collect.py", 80, "_options_frame_handler._handler", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Receives a decoded LEVELONE_OPTIONS/OPTIONS_BOOK frame on the daemon's stream loop and hands it to the bounded queue. O(1) and error-contained by construction: no field is read, parsed or derived here, because any work on this thread stalls LEVELONE_EQUITIES/CHART_EQUITY."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 228, "apply_options_slice", "NONE", None, (), None, "No market-field derivation: reconciles the live SUBSCRIPTION to the slice plan from options_desired_for_slice, ordering unsubscribe-then-close_epochs and subscribe-then-open_epochs so the coverage record never claims observability the vendor did not grant."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 442, "_options_rotation_loop", "NONE", None, (), None, "No market-field derivation: advances the rotation at clock-derived slice boundaries on the daemon's event loop."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 293, "_reconcile_options_subscription", "NONE", None, (), None, "No market-field derivation: performs the vendor unsubscribe/subscribe and the coverage epoch close/open in the order the record depends on. Split out of apply_options_slice so that ordering can be driven in a test without the locking and budget re-derivation around it."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 388, "_reconcile_options_subscription._underlying", "NONE", None, (), None, "No market-field derivation: maps a held option symbol to its underlying root for the ADMITTED per-underlying coverage tally, from the plan's symbol map with an option-root fallback for symbols stranded by a prior slice."),
    Mega2TraceableDerivation("options_stream_collect.py", 395, "_reconcile_options_subscription._by_underlying", "NONE", None, (), None, "No market-field derivation: tallies held option symbols by underlying root for the per-service ADMITTED coverage counts."),
