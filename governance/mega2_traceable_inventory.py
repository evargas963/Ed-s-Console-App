...
MEGA2_TRACEABLE_INVENTORY: tuple[Mega2TraceableDerivation, ...] = (
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 89, "_log_stream", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_log_stream)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 97, "_streaming_healthy", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (_streaming_healthy)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 108, "is_order_flow_stream_running", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (is_order_flow_stream_running)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 112, "get_plane_authority_for_ticker", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_plane_authority_for_ticker)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 129, "streaming_l1_cache_usable", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Fast-quote gate: plane row fresh within FAST_QUOTE_STREAM_CACHE_MAX_AGE_MS (streaming_l1_cache_usable)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 139, "get_streaming_diagnostics", "ALLOWLISTED", None, (), 'mega1_diagnostic_log', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_streaming_diagnostics)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 159, "_open_capture_db_readonly", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "SINGLE-STREAM-AUTHORITY repair 2026-08-30: read-only capture-DB handle for the live-plane feed, replacing the retired second StreamClient (_open_capture_db_readonly)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 175, "_replay_new_rows", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Replays daemon-captured L1/book rows into order_flow_live_state/live_market_plane, the same plane-ingest calls the retired direct-socket handlers made (_replay_new_rows)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 216, "_replay_option_contract_rows", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Same replay shape as _replay_new_rows for the one active option contract's LEVELONE_OPTIONS/OPTIONS_BOOK rows (_replay_option_contract_rows)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 252, "_feed_loop", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Poll loop replacing the retired StreamClient message loop; opens zero Schwab connections (_feed_loop)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 284, "set_streaming_active_ticker", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (set_streaming_active_ticker)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 307, "set_active_option_contract", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Requests LEVELONE_OPTIONS/OPTIONS_BOOK for one option contract via the daemon signal file (set_active_option_contract)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 330, "get_option_contract_book_microstructure", "DERIVED", None, ("order_flow_engine.py:compute_book_microstructure",), None, "Order-flow semantic product for one option contract's live book — delegates to the SAME producer the equity route reads, never a second book-imbalance computation (get_option_contract_book_microstructure)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 345, "start_order_flow_stream", "DERIVED", None, ("order_flow_live_state.py:get_content_for_symbol",), None, "Delegates to Schwab transport producers for start_order_flow_stream."),
    Mega2TraceableDerivation("order_flow_streaming.py", 372, "stop_order_flow_stream", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (stop_order_flow_stream)."),
    Mega2TraceableDerivation("order_flow_streaming.py", 388, "get_stream_thread", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Schwab LEVEL_ONE/stream book fields ingested via streaming adapter (get_stream_thread)."),
