...
MEGA2_TRACEABLE_INVENTORY: tuple[Mega2TraceableDerivation, ...] = (
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 83, "_options_frame_handler", "NONE", None, (), None, "No market-field derivation: builds the per-service options frame handler. Nested factory; the handler it returns owns the behaviour."),
    Mega2TraceableDerivation("order_flow_streaming.py", 92, "_options_frame_handler._handler", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Receives a decoded LEVELONE_OPTIONS/OPTIONS_BOOK frame on the shared stream loop and hands it to the bounded queue. O(1) and error-contained by construction: no field is read, parsed or derived here, because any work on this thread stalls LEVELONE_EQUITIES/NASDAQ_BOOK/NYSE_BOOK."),
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 282, "_options_rotation_loop", "NONE", None, (), None, "No market-field derivation: advances the rotation at clock-derived slice boundaries on the stream's own event loop."),
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 293, "_reconcile_options_subscription", "NONE", None, (), None, "No market-field derivation: performs the vendor unsubscribe/subscribe and the coverage epoch close/open in the order the record depends on. Split out of _apply_options_slice so that ordering can be driven in a test without the locking and budget re-derivation around it."),
    ...
    Mega2TraceableDerivation("order_flow_streaming.py", 483, "_reconcile_options_subscription._underlying", "NONE", None, (), None, "No market-field derivation: maps a held option symbol to its underlying root for the ADMITTED per-underlying coverage tally, from the plan's symbol map with an option-root fallback for symbols stranded by a prior slice."),
    Mega2TraceableDerivation("order_flow_streaming.py", 491, "_reconcile_options_subscription._by_underlying", "NONE", None, (), None, "No market-field derivation: tallies held option symbols by underlying root for the per-service ADMITTED coverage counts."),
