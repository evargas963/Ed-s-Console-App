...
MEGA2_TRACEABLE_INVENTORY: tuple[Mega2TraceableDerivation, ...] = (
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 98, "options_streaming_enabled", "NONE", None, (), None, "No market-field derivation: reads the ED_OPTIONS_STREAM gate at call time so enabling/disabling options collection is honoured without an import-time cache."),
    Mega2TraceableDerivation("options_stream_collect.py", 179, "register_options_handlers", "NONE", None, (), None, "No market-field derivation: attaches LEVELONE_OPTIONS/OPTIONS_BOOK handlers to the daemon's EXISTING StreamClient. Registration is inert until something subscribes, and failures are contained so the equity path cannot be aborted."),
    Mega2TraceableDerivation("options_stream_collect.py", 628, "start_options_collection", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Reconciles orphaned coverage, wires the daemon bus, subscribes the two options services on the daemon's sole client, and runs the rotation. Reads no Schwab market field itself; selection rides stored chain data."),
    Mega2TraceableDerivation("options_stream_collect.py", 696, "quiesce_options_collection", "NONE", None, (), None, "No market-field derivation: PHASE ONE of teardown — cancels AND awaits the rotation task and unsubscribes the vendor while the pump is still live; deliberately does NOT close coverage, so an in-flight frame cannot be cut off before the pump is quiesced."),
    Mega2TraceableDerivation("options_stream_collect.py", 736, "close_options_coverage", "NONE", None, (), None, "No market-field derivation: PHASE TWO of teardown — closes every open coverage epoch and resets state, run only AFTER the daemon has quiesced the pump and drained the writer, so no frame arrives past the close."),
    Mega2TraceableDerivation("options_stream_collect.py", 776, "stop_options_collection", "NONE", None, (), None, "No market-field derivation: combined teardown (quiesce then close) for paths where the pump is already quiesced — a recycle or a failed start. The daemon shutdown path calls quiesce and close separately to order the coverage close after the writer drain."),
    Mega2TraceableDerivation("options_stream_collect.py", 789, "options_stream_status", "NONE", None, (), None, "No market-field derivation: reports options collection enablement, subscribed contract count and the offered/written frame counters for the diagnostics surface."),
    Mega2TraceableDerivation("options_stream_collect.py", 110, "_options_frame_handler", "NONE", None, (), None, "No market-field derivation: builds the per-service options frame handler. Nested factory; the handler it returns owns the behaviour."),
    Mega2TraceableDerivation("options_stream_collect.py", 120, "_options_frame_handler._handler", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Receives a decoded LEVELONE_OPTIONS/OPTIONS_BOOK frame on the daemon's stream loop and PUBLISHES it onto the daemon bus (the single CaptureWriter persists it). O(1) and error-contained by construction: no field is read, parsed or derived here, because any work on this thread stalls LEVELONE_EQUITIES/CHART_EQUITY."),
    Mega2TraceableDerivation("options_stream_collect.py", 140, "make_capture_topic_writer", "NONE", None, (), None, "No market-field derivation: builds the persister the daemon registers on its single CaptureWriter for the option topic kinds. Nested factory; the returned _write owns the behaviour."),
    Mega2TraceableDerivation("options_stream_collect.py", 153, "make_capture_topic_writer._write", "ALLOWLISTED", None, (), 'mega2_schwab_stream_l1', "Persists one decoded options frame onto the CaptureWriter's OWN connection (options_stream_frames + the per-contract index) using the shared row shaping, so options rides the single writer and never opens a second connection to stream_capture.db. Reads no field: it stores the decoded frame whole."),
    Mega2TraceableDerivation("options_stream_collect.py", 599, "flush_options_ingest_health", "NONE", None, (), None, "No market-field derivation: records one options_stream_ingest_health window from the offered/written counters at teardown, so the run's drop accounting (offered - written) survives the process for the SUBSCRIBED_MAYBE_DROPPED verdict."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 315, "apply_options_slice", "NONE", None, (), None, "No market-field derivation: reconciles the live SUBSCRIPTION to the slice plan from options_desired_for_slice, ordering unsubscribe-then-close_epochs and subscribe-then-open_epochs so the coverage record never claims observability the vendor did not grant."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 574, "_options_rotation_loop", "NONE", None, (), None, "No market-field derivation: advances the rotation at clock-derived slice boundaries on the daemon's event loop."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 387, "_reconcile_options_subscription", "NONE", None, (), None, "No market-field derivation: performs the vendor unsubscribe/subscribe and the coverage epoch close/open in the order the record depends on. Split out of apply_options_slice so that ordering can be driven in a test without the locking and budget re-derivation around it."),
    ...
    Mega2TraceableDerivation("options_stream_collect.py", 515, "_reconcile_options_subscription._underlying", "NONE", None, (), None, "No market-field derivation: maps a held option symbol to its underlying root for the ADMITTED per-underlying coverage tally, from the plan's symbol map with an option-root fallback for symbols stranded by a prior slice."),
    Mega2TraceableDerivation("options_stream_collect.py", 522, "_reconcile_options_subscription._by_underlying", "NONE", None, (), None, "No market-field derivation: tallies held option symbols by underlying root for the per-service ADMITTED coverage counts."),
