...
| `EdConsole Stream Capture` | Daily 08:00 ET, duration 0 (until process death) | `cmd /c cd /d <REPO> && .venv\Scripts\python.exe tools\run_stream_capture.py --symbols SPY,QQQ,IWM --duration-min 0` | `data/stream_capture.lock` + heartbeat in `stream_capture.db` | 2026-09-03 — finite 405-min run retired; watchdog below keeps it up |
...
| `EdConsole Stream Capture Watchdog` | Every 1 min | `<REPO>\tools\ensure_stream_capture.bat` (pythonw + sibling live DB path) | stdout JSON `already_running`/`started` | 2026-09-03 — 5-min tick retired; 30s heartbeat TTL needs a 1-min host poll |
