...
| `EdConsole Stream Capture` | Weekdays 08:25 ET, duration 0; same-task 1-min repetition for 12h (IgnoreNew; StopAtDurationEnd=false); ExecutionTimeLimit unlimited | `<REPO>\.venv\Scripts\pythonw.exe -m app.market_data.schwab.streaming.capture --duration-min 0` (2026-09-23: no `--symbols` — the daemon streams exactly what the console requests; an old `--symbols SPY,QQQ,IWM` still runs, as a boot set only) (WorkingDirectory=<REPO>) | DB-adjacent `stream_capture.lock` + heartbeat in `stream_capture.db` + `reports/stream_capture_status.json` | 2026-09-08 — **OPERATOR MUST RE-REGISTER before the candidate is the stream owner:** the live host task still calls `-m tools.run_stream_capture`, which PR #224 deletes (module moved to `app.market_data.schwab.streaming.capture`). One owner, no watchdog; the host 1-min repetition is the restart authority (RC: false-success watchdog deleted). |
| `EdConsole Database Backup` | Daily 18:30 ET | `<REPO>\.venv\Scripts\python.exe -m tools.backup_permanent_databases --reason scheduled_refresh` | stable `backups/db/ed_console_backup_manifest.json` + `stream_capture_backup_manifest.json`; non-zero task result on any source/backup/validation failure | **NOT YET REGISTERED** — registration and on-demand execution are merge acceptance steps; do not register a task whose code is not on main |
...
| ~~`EdRehabDailyScan`~~ | RETIRED (bedrock 2026-09-06) | ~~`powershell.exe ... run_rehab_daily.ps1`~~ — the launcher, the scan (`tools/rehab_daily_scan.py`), the TQM queue and the advisory-debt report were DELETED with the advisory-debt loop | (artifacts deleted from git) | **OPERATOR TO DELETE the host task:** `schtasks /Delete /TN "EdRehabDailyScan" /F`, then verify ABSENT via `Get-ScheduledTask`. Tombstone kept per this file's honest-record rule; until deleted the task fails on a missing script, which is visible in its Last Result. |
