...
| `EdConsole Stream Capture` | Daily 08:25 ET, 405 min | `<REPO>\.venv\Scripts\pythonw.exe -m tools.run_stream_capture --symbols SPY,QQQ,IWM --duration-min 405`, working directory `<REPO>` | `data/stream_capture.lock` owner + `reports/stream_capture_status.json` | 2026-09-07 — options-chain worktree action stopped and replaced on host; task verified Ready with canonical executable + working directory |
