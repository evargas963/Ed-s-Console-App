...
| `EdRthCompletenessCheck` | daily | `<REPO>\.venv\Scripts\pythonw.exe <REPO>\tools\rth_completeness_check_v1.py --db <REPO>\data\ed_console.db --backfill` (**pythonw**, not python — the tool now leaves a durable final record, so a windowless run loses no visibility; re-registration command below) | `reports/rth_validation/rth_completeness_latest.json` (durable final record, written on EVERY exit path; fail-closed exit 0/1/2 unchanged) | 2026-08-28 — observability root-fixed (durable record + progress + `completion_path`); **operator to re-register with `pythonw` per the command below**. Prior `python.exe` form measured 2026-08-25 Ready; 2026-08-04 Last Result was **3221225786** |
...
`HOLES` reads as an intermediate step, never the verdict. Fail-closed exit codes are unchanged
(0 = complete / no-session; 1 = holes stand or real vendor loss; 2 = unmeasurable, never a pass —
RC-57). No new checker/ledger/registry/governance file; the record is a run report in the existing
`reports/rth_validation/` location.
