...
| **T3 Stem / full** | Nightly Automation or pre-release only | the full suite via `npm run test:all` (its pytest step is `node scripts/run-pytest-full.mjs`, RC-535 output sink; append `-n 8` to pin workers) — **not** every mission turn |
