...
| R-01 | server.py get_chain (/api/chain) | a failed live chain fetch (vendor non-200, error, or a different expiry returned) was answered from, in turn, the vendor's other expiry, an older persisted complete capture, or a bounded stored snapshot that could belong to a different expiry -- labeled, still a substitute; SPY's expired 0DTE drew HTTP 400 and was served from storage on every refresh (measured 2026-09-24) | Chain ladder, Strike Detail, contract specs, /options | FIXED (this change: live complete chain or `unavailable` + reason) |
