...
| CR-03 | CR-CAP | WS hub + main chart panel; **retire polling loops + demote/hide chance-level ML DOM** | ≥1 legacy polling loop retired; ML surfaces hidden (hard-delete deferred to §8.3); layout-contract tests. Panels registry + volume profile POC/VA are **deferred to CR-03b** after the shell is stable — no registry indirection before capture is proven |
