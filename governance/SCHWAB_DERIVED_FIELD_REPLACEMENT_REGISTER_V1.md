...
| DFR-017 | Inline `chain_row.get("multiplier")` reads at consumer sites including `math_exposure_core.py::compute_exposures_by_strike()` (formerly also `chains.py::contract_fields()` — removed in the Schwab-direct redesign) | `multiplier` defaulted to `100` | Missing Schwab multiplier silently coerces non-standard contracts into standard-contract dollarized exposure math. | gate/fail-closed / replace-with-Schwab | High |
