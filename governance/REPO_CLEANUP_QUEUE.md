...
| `realized_contract_eval.py:846,961,990,1013` | `realized_contract_eval` still reads raw option-chain dict fields for replay selection and entry/exit pricing instead of routing through `chains.contract_fields()` or an equivalent normalized accessor. | 2026-05-06 | Normalize replay contract access at the boundary before using these rows for durable A2 label training or replay/live parity gates. | open | Gap ID: `realized_contract_eval_raw_chain_reads_pending_normalization`; A2 replay-label sidecar consumes trade-log rows only and does not launder this upstream raw path. |
