"""CLI wrapper for manual A1 isotonic artifact production."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Produce an A1 isotonic artifact manually.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        result = produce_a1_isotonic_artifact(
        ...
        logging.exception("A1 isotonic artifact production failed")
        return 2

    logging.info(
        "A1 isotonic artifact production status=%s pointer_updated=%s artifact_path=%s eligibility_reason=%s",
        result["status"],
