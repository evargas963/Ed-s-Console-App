...
# ─────────────────────────────────────────────────────────────────────────────
# GAMMA-SURFACE LAST-VALID SNAPSHOT (operator directive, 2026-09-15) — the canonical
# input-validity rules' persisted durability layer, a SIBLING of option_chain_accrual above,
# same DB file, same module, same conventions -- not a second, disconnected persistence
# authority. option_chain_accrual banks the per-STRIKE aggregate (collapsed across every
# expiry, one row per observation instant); the heatmap needs the LATEST valid value per
# (strike, EXPIRY) CELL specifically, which that shape cannot serve -- this table is the
# per-cell analogue, always-latest (PRIMARY KEY replaces, not a time series) since a snapshot
# fallback only ever needs the newest known-good value, never a history of them.
# ─────────────────────────────────────────────────────────────────────────────
...
def ensure_gamma_last_valid_schema(conn: sqlite3.Connection) -> None:
    conn.executescript(GAMMA_LAST_VALID_TABLE_SQL)
    conn.commit()


def load_gamma_surface_last_valid(db_path: Path | str, ticker: str) -> dict[tuple[float, str], dict]:
    """Every persisted last-known-valid cell for `ticker`, keyed (strike, expiry) exactly
    like server.py's in-memory _LAST_VALID_GEX_CELLS -- this IS that dict's durable backing,
    read once to rehydrate it after a process restart. Fail-closed: a missing file, missing
    table, or any sqlite error returns {} (cold start behaves exactly as it always did before
    this table existed -- '—' until the first real value, never an exception surfaced to a
    request handler)."""
    path = Path(db_path)
    if not path.is_file():
        return {}
    ...
    try:
        conn = sqlite3.connect(f"file:{path.resolve().as_posix()}?mode=ro", uri=True, timeout=10.0)
    except sqlite3.Error:
        return {}
    try:
        if not conn.execute(
    ...
    for strike, expiry, gex, dex, vanna, ts in rows:
        out[(float(strike), str(expiry))] = {
            "gex": gex, "dex": dex, "vanna": vanna, "captured_ts_utc": float(ts),
        }
    return out


def persist_gamma_surface_last_valid(
    db_path: Path | str, *, ticker: str, cells: list[dict],
) -> dict[str, Any]:
    """Durably upsert the CURRENTLY-valid cells only -- one batched transaction, never one
    write per cell. `cells` is `[{"strike", "expiry", "gex", "dex", "vanna",
    "captured_ts_utc"}, ...]`. FAIL CLOSED: empty input writes nothing; a write failure is
    logged by the caller (persistence is a durability enhancement, never load-bearing for the
    live response the in-memory dict already served)."""
    tk = ticker_storage_key(ticker)
    if not tk or not cells:
        return {"status": "skipped", "reason": "no_ticker_or_cells"}
    rows = [
    ...
    conn = sqlite3.connect(str(db_path), timeout=60.0)
    try:
        ensure_gamma_last_valid_schema(conn)
        conn.executemany(
        ...
        conn.close()
    return {"status": "written", "ticker": tk, "n_rows": len(rows)}
