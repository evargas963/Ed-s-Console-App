...
#: every windowed cf_* declares its span in MINUTES, the unit its NAME asserts. These
#: windows were slot counts until 2026-08-09. A slot count is not a duration: the same
...
#: previous session — which is precisely the 90.4-hour reach-back that motivated this.
...
def compute_confluence_features(snapshots_5m: list[dict], current_idx: int) -> dict:
    ...
    position. Callers may therefore pass any row spacing and any filtering — RTH-only
    training frames and unfiltered serve reads included — and get the same value for the
    same bar. A window whose span is not present in the supplied rows reports ABSENT rather
...
def build_lstm_dataset(
    ...
    nan_counts = {}
    unique_days = set()
    # One UTC-day pool of canonical history per ticker, reused across the slide.
    ...
    for ticker in tickers:
        ...
        for day_key, snapshots in sorted(days_data.items()):
            ...
            for end_idx in range(STREAM_5M_LOOKBACK, n_snaps):
                ...
                # The BAR comes from ``snapshots``, but the confluence HISTORY must not.
