...
# test_lstm_requests_edge_pp_not_val_accuracy, test_producer_slice_has_no_arithmetic_
# translation_ast and test_producer_slice_call_surface_is_a_closed_whitelist were retired
# (RC-REHAB-1, 2026-09-23): all three sliced server.py's `_model_status_from_artifact`
# out of the source text, and that producer (the per-cycle Model Health Dashboard) was
# deleted outright -- zero consumers anywhere, and the retired-consumer note below had
# already recorded that the /console rebuild has no model-health surface. The three
# string locks above still forbid a fabricated zero edge anywhere in server.py.
