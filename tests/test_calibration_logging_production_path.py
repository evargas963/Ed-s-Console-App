...
def test_compute_signals_writes_one_calibration_row_per_successful_decision(
    calib_db: Path,
    monkeypatch: pytest.MonkeyPatch,
    stub_parallel_stack_for_calibration_proofs: None,
) -> None:
    """Event parity: N successful compute_signals completions → N new calibration rows."""
    monkeypatch.setenv("ED_CALIBRATION_LOG", "1")
    edb = EdDB(calib_db)
    ...
    for i in range(n):
        ...
        ts_list.append(rts)
        out = compute_signals(_inp(refresh_ts_utc=rts), db=edb)
...
def test_decision_ts_utc_matches_refresh_ts_utc(
    ...
    compute_signals(_inp(refresh_ts_utc=rts), db=edb)
...
def test_no_duplicate_ticker_decision_ts_pairs_for_distinct_refreshes(
    ...
    base = time.time()
    for i in range(8):
        compute_signals(_inp(refresh_ts_utc=base + i * 0.1), db=edb)
...
def test_concurrent_identical_decision_key_single_row(
    ...
    def run_same() -> None:
        try:
            compute_signals(_inp(refresh_ts_utc=shared_ts), db=edb)
        except BaseException as e:
            errors.append(e)
