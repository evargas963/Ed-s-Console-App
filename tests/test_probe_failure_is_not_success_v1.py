"""RC-506 — a measurement that did not run is not a measurement that came back clean.

THE CLASS. A gate shells out to git (or ruff, or pytest), keeps only the child's STDOUT, and
decides from that text. When the child exits non-zero it usually prints nothing, so the gate
reads an empty string and takes the same branch it takes for a genuinely clean result. Green
and "never looked" become byte-identical, which is the same shape as a waiter that treats the
absence of a summary line as "still running".

MEASURED 2026-09-02, running the real unmodified `tools/check_live_path_is_main.py` in a
throwaway repo on branch main, clean tree, with no resolvable `origin/main`:

    ahead  probe: rc=128 stdout=''
    behind probe: rc=128 stdout=''
    guard stdout: ONE-APP LOCK: PASS - the running app is a provable build of origin/main.
    GUARD EXIT CODE: 0

That is the desk-launch gate certifying a lineage it had not measured. Verifying it also
...
def test_an_unmeasurable_lineage_refuses_the_launch(tmp_path, monkeypatch):
    """origin/main unresolvable, clean tree, on main. The guard must REFUSE, not certify."""
...
def test_negative_control_the_old_code_path_was_fail_open_on_this_very_repo(tmp_path, monkeypatch):
    ...
    therefore silently clean. Patching only the helper leaves the new caller in place and
    produces a garbled violation instead of the original silence — a control that fails for
    the wrong reason. So the old path is reproduced verbatim here and run against the same
    repository state the real guard is asked about."""
    ...
    assert CLP.violations(), (
        "the real guard does not refuse the state the old path waved through — the fix is "
        "not load-bearing")
...
def test_negative_control_the_old_strip_drops_the_first_filenames_leading_char(tmp_path):
    """Isolates the truncation itself, with no dependence on the guard's wiring."""
