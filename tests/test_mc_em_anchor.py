...
def test_server_wires_mc_iv_level_into_build_market_state():
    # RC-REHAB-1 (thirty-fifth slice): the build_market_state call is in server_state_decision.py.
    source = open("server_state_decision.py", encoding="utf-8").read()
    assert "mc_iv_level=em.mc_iv_level" in source
    assert "mc_em_anchor=em.kl_em_anchor" in source
    # RC-REHAB-1 (thirty-third slice): the payload projection moved to server_state_payload.py.
    assert 'ms_dict["mc_em_anchor"]' in open("server_state_payload.py", encoding="utf-8").read()
