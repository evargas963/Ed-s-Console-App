...
def _patch_all(vol_oi_ratio, flow_imb, smart_money, iv_model_spread):
    return (
        mock.patch.object(srv, "compute_volume_oi_ratio", vol_oi_ratio),
        mock.patch.object(srv, "flow_imbalance_normalized_with_fallback", flow_imb),
        mock.patch.object(srv, "compute_smart_money_signal", smart_money),
        mock.patch.object(srv, "compute_iv_model_spread", iv_model_spread),
    )


def test_happy_path_all_four_signals_populate():
    with mock.patch.object(srv, "compute_volume_oi_ratio", lambda e, s: {"ratio": 1.5, "label": "elevated"}), \
         mock.patch.object(srv, "flow_imbalance_normalized_with_fallback", lambda e, s: (0.42, "volume")), \
         mock.patch.object(srv, "compute_smart_money_signal", lambda e, s: {"score": 7, "direction": "bullish", "label": "strong"}), \
         mock.patch.object(srv, "compute_iv_model_spread", lambda c, s: {"spread": 0.03, "label": "tight"}):
...
def test_one_signal_failing_does_not_affect_its_siblings():
    ...
    with mock.patch.object(srv, "compute_volume_oi_ratio", boom), \
         mock.patch.object(srv, "flow_imbalance_normalized_with_fallback", lambda e, s: (0.42, "volume")), \
         mock.patch.object(srv, "compute_smart_money_signal", boom), \
         mock.patch.object(srv, "compute_iv_model_spread", lambda c, s: {"spread": 0.03, "label": "tight"}):
        result = srv._order_flow_signals_for_state({"x": 1}, 450.0, [{"strike": 450}])
...
def test_all_four_signals_failing_never_raises():
    ...
    with mock.patch.object(srv, "compute_volume_oi_ratio", boom), \
         mock.patch.object(srv, "flow_imbalance_normalized_with_fallback", boom), \
         mock.patch.object(srv, "compute_smart_money_signal", boom), \
         mock.patch.object(srv, "compute_iv_model_spread", boom):
