"""regime_engine chunk-1: FIND-RE1 VWAP contra message + I-01 unknown fallback."""

from __future__ import annotations

from types import SimpleNamespace

from regime_engine import _score_trend_continuation, _unknown_regime, _micro_regimes
from signal_types import RulesCard, SignalInput
from tests.mvp_test_fixtures import minimal_mvp_features


def _minimal_inp() -> SignalInput:
    return SignalInput(
        ticker="SPY",
        timeframe="1m",
        expiry=None,
...
def test_trend_continuation_no_vwap_contra_when_signal_wait():
    """FIND-RE1: wait + above VWAP must not emit a VWAP conflict contra string."""
    mr = _micro_regimes()
    rules = RulesCard(
        headline="",
        headline_1m="",
        ...
        conviction="low",
        alerts=[],
        micro=SimpleNamespace(regime=mr["chop"]),
    )
    mvp = minimal_mvp_features(vwap_side="above")

    _score, _support, contra = _score_trend_continuation(
        _minimal_inp(), mr["chop"], mr, rules, mvp
    )

    assert not any("VWAP side" in c and "conflicts" in c for c in contra)


def test_unknown_regime_fail_closed():
    payload = _unknown_regime()
    assert payload.primary == "unknown"
