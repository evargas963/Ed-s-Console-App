...
def test_five_status_vocabulary_includes_not_applicable():
    assert VALID_PROOF == {
        "PASS", "FAIL", "NOT_PROVEN", "UNAVAILABLE", "NOT_APPLICABLE",
    }
    by = {"X": {"id": "X", "proof": "NOT_APPLICABLE", "closable": True}}
    assert compute_item_proof(by["X"], by) == "NOT_APPLICABLE"


def test_all_not_applicable_children_yield_not_applicable():
    tree = _tree([
        {"id": "PARENT", "proof": "NOT_PROVEN", "closable": True,
         "children": ["C1", "C2"]},
        {"id": "C1", "proof": "NOT_APPLICABLE"},
        {"id": "C2", "proof": "NOT_APPLICABLE"},
    ])
    by = {i["id"]: i for i in tree["items"]}
    assert compute_item_proof(by["PARENT"], by) == "NOT_APPLICABLE"
...
def test_live_tree_parents_not_closed_by_children():
    ...
    assert by["OF_COMPOSITE_RETIRED"]["proof"] == "NOT_PROVEN"
    assert by["IDENTITY_SPLIT_REMOVED"]["proof"] == "NOT_PROVEN"
