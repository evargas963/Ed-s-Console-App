...
def test_memory_does_not_advance_when_the_coverage_open_write_fails(monkeypatch):
    from calibration.options_stream_coverage import CoverageWriteError

    monkeypatch.setattr(osc, "_options_subscribed",
                        {s: set() for s in osc.OPTIONS_SERVICES}, raising=False)
    want = ["SPY   260820C00600000"]
    ...
    asyncio.run(_run())
    assert not any(osc._options_subscribed.values()), (
        "the vendor accepted the subscribe but the durable epoch open FAILED, yet memory marked "
        "the contract subscribed — memory is claiming coverage the record does not have")


def test_memory_is_retained_when_the_coverage_close_write_fails(monkeypatch):
    from calibration.options_stream_coverage import CoverageWriteError

    held = {"LEVELONE_OPTIONS": {"SPY   260820C00600000"}, "OPTIONS_BOOK": set()}
    monkeypatch.setattr(osc, "_options_subscribed", held, raising=False)
    ...
    asyncio.run(_run())
    assert osc._options_subscribed["LEVELONE_OPTIONS"] == {"SPY   260820C00600000"}, (
        "the vendor unsubscribe was acknowledged but the durable close FAILED, yet memory dropped "
        "the contract — memory now claims LESS coverage than the still-open epoch, and nothing "
