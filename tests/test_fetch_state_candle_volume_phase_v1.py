...
def test_accumulator_primary_used_directly_without_price_history_call():
    ...
    try:
        with mock.patch.object(sc, "safe_get_price_history") as mock_ph:
...
def test_falls_back_to_price_history_when_accumulator_has_no_bars():
    ...
    try:
        ...
        with mock.patch.object(sc, "safe_get_price_history", return_value=resp) as mock_ph:
...
def test_both_sources_unavailable_yields_none_without_raising():
    orig = _with_bars([])
    try:
        with mock.patch.object(sc, "safe_get_price_history", return_value=None):
...
def test_price_history_exception_is_swallowed_falls_through_to_none():
    orig = _with_bars([])
    try:
        with mock.patch.object(sc, "safe_get_price_history", side_effect=RuntimeError("boom")):
