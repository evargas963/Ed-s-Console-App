...
def test_mega1_scope_complete():
    ...
    assert len(MEGA1_TRACEABLE_INVENTORY) == 550  # +4 options-flow production (3 lifecycle + api_options_history)  # +1 RC-514 2026-09-03 (server.schwab_capability_state: /api/health capability verdict, DERIVED from schwab_client.build_client_from_token)  # +1 OPTIONS_ORDER_FLOW_V1 round 4 2026-08-31 (_persist_universal_complete_chain: systematic near-term complete-chain capture)  # +1 OPTIONS_ORDER_FLOW_V1 options_page 2026-08-30 (serves static/options.html, UI/consumer wiring)  # +1 OPTIONS_ORDER_FLOW_V1 get_chain 2026-08-30 (contract-selection chain API, DERIVED from server.py:_latest_chain_and_spot)  # +3 OPTIONS_ORDER_FLOW_V1 order-flow API 2026-08-30 (api_order_flow_options_microstructure, post_streaming_active_option_contract, post_streaming_active_option_contract._apply)  # +1 null_pa_vwap_zscore_roll_contamination (VWAP semantic fidelity) 2026-08-29  # -1 production-coherence-v1 2026-08-29 (server._attach_stack_runtime_and_governance._layer_ns deleted: server consumes the producer authorization verdict instead of re-deriving it from layer shape) # +2 -1 Cursor-audit 2026-08-25 (_chain_from_date_for F2 near-edge bound, _enrollment_collectability_probe F5 enroll gate; -1 removed nested get_terrain_strikes._per_strike._dte, replaced by canonical terrain_engine._dte_of F8)  # +1 RC-494 2026-08-25 (_chain_to_date_for: index chain date bound)  # -1 RC-493 2026-08-25 (_operator_mode_cycle_roster removed: universal collection is unconditional)  # +5 RC-484/RC-69 2026-08-25 (safe_get_daily_price_history; _enrollment_history_seed, _persist_1m_bars, _radar_daily_atr_vendor_fallback)  # +1 api_order_flow_microstructure (ORDER_FLOW_MARKET_MICROSTRUCTURE_V1 route) 2026-08-21  # RC-416: +1 carried_price_levels_match_snapshot  # RC-365: +2 F39 confluence mapper (market_context) +3 Phase 2A server fns (_garch_horizon_bars, _canonical_price_level_bars, canonical_price_level_snapshot) −3 stale (deleted _compute_vwap_from_bars, get_levels nested pair)  # +18 streaming/radar/tier-c/terrain-strikes endpoints (server.py) 2026-07-26  # +15 terrain/shutdown infra 2026-07-20 (watchdog, signals, strike geometry, ATR cache, radar rows, reprice)  # +1 _last_traded_price (RC-18) 2026-07-19  # +1 spot_is_a_close (RC-16) 2026-07-19  # +3 spot authority (RC-14) 2026-07-19  # +2 radar handler + snapshot merge 2026-07-19  # +8 terrain loop + chain helpers 2026-07-19  # +2 /api/terrain handler + _latest_chain_and_spot 2026-07-19 (prior: 433)  # inventory sync @ CHAIN_GATE_V2: -3 _PriorityGate +7 _ChainGateV2 +1 diagnostics endpoint
