...
def test_no_global_l1_generation_mutable_in_ed_core():
    ...
    ed-core.js paints no L1 projection at all since prices come from the daemon socket (its
    `_l1Gen`/`_l1Ts` dicts were declared and never read, and were deleted with the no-op
    l1_projection listener): there is no generation state to share across scopes."""
    text = (ROOT / "static" / "js" / "ed-core.js").read_text(encoding="utf-8")
    assert "addEventListener('l1_projection'" not in text
...
def test_authority_and_identity_stores_keyed_by_scope_in_source():
    """Header paint is the daemon price row scoped to state.ticker. A row for another symbol
    must not write #hPx. Watchlist rows key off the row's ticker against loadWL()."""
    text = (ROOT / "static" / "js" / "ed-core.js").read_text(encoding="utf-8")
    assert "function ingestPriceRow(q)" in text
    ...
    assert "wl.indexOf(sym)" in text and "var wl = loadWL();" in text
