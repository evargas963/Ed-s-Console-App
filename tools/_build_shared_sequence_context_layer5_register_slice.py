#!/usr/bin/env python3
"""Build governance/register_slices/shared_sequence_context_py_1_*.csv — Layer 5 chunk-1 walk."""
...
PATH = "features/shared_sequence_context.py"
DICT_PATH = ROOT / "schwab_field_inventory" / "schwab_field_dictionary.csv"
_CITE_RE = re.compile(r"CSV row (\d+) \(canonical_field=([^)]+)\)")
TRACE = "CLAUDE Layer 5 chunk-1 disposition features/shared_sequence_context.py"
ANCHOR = "line anchored HEAD c8703c0; 11/11 features sweep closure; slice-only"
...
FORMAL_KEEP_DERIVED: list[tuple[int, str, str]] = [
    (22, "_require_ticker", zero("_require_ticker")),
    (41, "_max_transformer_seq_len_for_ticker", zero("_max_transformer_seq_len_for_ticker")),
    (118, "build_shared_sequence_context", zero("build_shared_sequence_context")),
    (196, "transformer_window_chronological", zero("transformer_window_chronological")),
]

FORMAL_PASS_THROUGH: list[tuple[int, str, str]] = [
    (142, "db.get_recent_snapshots", "PASS_THROUGH EdDB persisted snapshots"),
    (169, "build_lstm_merged_windows", "PASS_THROUGH features.lstm_sequence_input"),
]
...
def main() -> None:
    ...
    slice_path = ROOT / "governance" / "register_slices" / f"shared_sequence_context_py_1_{hi}.csv"
    ...
    for line in range(1, hi + 1):
        ...
        row = _synth_row(
            ...
            "NOT_MARKET_DATA",
            notes="Shared LSTM/TR sequence prep (one DB fetch per tick)",
        )
        col += 1
        out_by_id[row["register_id"]] = row
