#!/usr/bin/env python3
"""Build governance/register_slices/mvp_source_coercion_py_1_*.csv — Layer 5 chunk-1 walk."""
...
PATH = "features/mvp_source_coercion.py"
DICT_PATH = ROOT / "schwab_field_inventory" / "schwab_field_dictionary.csv"
_CITE_RE = re.compile(r"CSV row (\d+) \(canonical_field=([^)]+)\)")
TRACE = "CLAUDE Layer 5 chunk-1 disposition features/mvp_source_coercion.py"
ANCHOR = (
    "line anchored HEAD c9a185e; FIND-MSC1 _require_mapping non-Mapping parent fail-closed"
)
...
FORMAL_KEEP_DERIVED: list[tuple[int, str, str]] = [
    (27, "_contains_key TypeError-safe membership", zero("_contains_key")),
    (34, "_require_mapping parent Mapping gate (FIND-MSC1)", zero("_require_mapping")),
    (42, "strict_float_from_raw strict numeric coercion", zero("strict_float_from_raw")),
    (61, "read_optional_float missing-vs-invalid reader", zero("read_optional_float")),
    (75, "read_optional_zone vocab-enforced reader", zero("read_optional_zone")),
    (98, "read_optional_vwap_side vocab-enforced reader", zero("read_optional_vwap_side")),
    (121, "read_liquidity_summary_subdict container reader", zero("read_liquidity_summary_subdict")),
]
...
def main() -> None:
    ...
    slice_path = ROOT / "governance" / "register_slices" / f"mvp_source_coercion_py_1_{hi}.csv"
    ...
    for line in range(1, hi + 1):
        ...
        row = _synth_row(
            ...
            "NOT_MARKET_DATA",
            notes="MVP source coercion boundary (no Schwab leaves)",
        )
        col += 1
        out_by_id[row["register_id"]] = row
