#!/usr/bin/env python3
"""Build governance/register_slices/canonical_contract_py_1_346.csv — Layer 5 chunk-1 walk (slice-only)."""
...
SLICE = ROOT / "governance" / "register_slices" / "canonical_contract_py_1_346.csv"
DICT_PATH = ROOT / "schwab_field_inventory" / "schwab_field_dictionary.csv"
_CITE_RE = re.compile(r"CSV row (\d+) \(canonical_field=([^)]+)\)")
TRACE = "CLAUDE Layer 5 chunk-1 disposition features/canonical_contract.py 1-346"
PATH = "features/canonical_contract.py"
LO, HI = 1, 346
ANCHOR = (
    "line anchored HEAD 3e592f1; upstream MVP choke-point validator; "
    "OBS-CC1/CC2 informational disclosures slice-only"
)
...
FORMAL_KEEP_DERIVED: list[tuple[int, str, str]] = [
    (251, "get_mvp_feature_names accessor", zero("get_mvp_feature_names")),
    (256, "get_feature_spec accessor", zero("get_feature_spec")),
    (263, "get_mvp_field_semantics accessor", zero("get_mvp_field_semantics")),
    (270, "validate_feature_contract_row choke-point validator", zero("validate_feature_contract_row")),
    (344, "excluded_from_mvp helper", zero("excluded_from_mvp")),
]
...
def main() -> None:
    ...
    for line in range(LO, HI + 1):
        ...
        row = _synth_row(
            ...
            "NOT_MARKET_DATA",
            notes="MVP canonical contract schema + spec data (no Schwab leaves)",
        )
        col += 1
        out_by_id[row["register_id"]] = row
