#!/usr/bin/env python3
"""Build governance/register_slices/lstm_sequence_input_py_1_238.csv — Layer 5 chunk-1 walk (slice-only)."""
...
SLICE = ROOT / "governance" / "register_slices" / "lstm_sequence_input_py_1_238.csv"
DICT_PATH = ROOT / "schwab_field_inventory" / "schwab_field_dictionary.csv"
_CITE_RE = re.compile(r"CSV row (\d+) \(canonical_field=([^)]+)\)")
TRACE = "CLAUDE Layer 5 chunk-1 disposition features/lstm_sequence_input.py 1-238"
PATH = "features/lstm_sequence_input.py"
LO, HI = 1, 238
ANCHOR = (
    "line anchored HEAD 6f410a7; Day3 c527b82 provenance; "
    "FIND-LSI1/LSI2 disclosed slice-only"
)
...
FORMAL_KEEP_DERIVED: list[tuple[int, str, str]] = [
    (63, "ZONE_MISSING_ENCODED / VWAP_SIDE_UNKNOWN_ENCODED sentinels", zero("ZONE_MISSING_ENCODED")),
    (82, "_canonical_missing_masks mask channel builder", zero("_canonical_missing_masks")),
    (86, "_patch_lstm_categoricals zone/vwap sentinels", zero("_patch_lstm_categoricals")),
    (107, "encode_lstm_structure_bar_with_masks", zero("encode_lstm_structure_bar_with_masks")),
    (123, "encode_lstm_micro_bar_with_masks", zero("encode_lstm_micro_bar_with_masks")),
    (135, "merge_db_row_with_canonical_mvp MVP strip + canonical override", zero("merge_db_row_with_canonical_mvp")),
    (149, "_ts_close timestamp proximity helper", zero("_ts_close")),
    (158, "build_lstm_merged_windows main producer", zero("build_lstm_merged_windows")),
    (217, "build_transformer_merged_window", zero("build_transformer_merged_window")),
]

FORMAL_PASS_THROUGH: list[tuple[int, str, str]] = [
    (144, "CANONICAL_TO_XGB_TABULAR merge into db row", "PASS_THROUGH canonical MVP feature names"),
    (189, "live-bar inference_snapshot_v1 features on last window bar", "PASS_THROUGH InferenceSnapshotV1 features"),
    (204, "day_snaps live-bar override by ts_utc match", "PASS_THROUGH InferenceSnapshotV1 features"),
    (190, "inference_snapshot_v1 features on last window index", "PASS_THROUGH InferenceSnapshotV1 features"),
]
...
def main() -> None:
    ...
    for line in range(LO, HI + 1):
        ...
        row = _synth_row(
            ...
            "NOT_MARKET_DATA",
            notes="LSTM/Transformer sequence canonical MVP contract (no Schwab leaves)",
        )
        col += 1
        out_by_id[row["register_id"]] = row
