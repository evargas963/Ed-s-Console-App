...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--fetch-github", action="store_true", help="Fetch via GitHub CLI (gh)")
    ...
    if args.fetch_github:
        evidence = fetch_github_cli_evidence()
        if evidence.get("verification_method") == "pending":
            print("GitHub CLI fetch failed or branch protection not configured — verified remains false", file=sys.stderr)
        write_all_artifacts(evidence, generated=date.today().isoformat())
