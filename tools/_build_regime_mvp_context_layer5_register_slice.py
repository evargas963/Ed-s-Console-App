#!/usr/bin/env python3
"""Build governance/register_slices/regime_mvp_context_py_1_62.csv — Layer 5 chunk-1 walk."""
...
SLICE = ROOT / "governance" / "register_slices" / "regime_mvp_context_py_1_62.csv"
DICT_PATH = ROOT / "schwab_field_inventory" / "schwab_field_dictionary.csv"
_CITE_RE = re.compile(r"CSV row (\d+) \(canonical_field=([^)]+)\)")
TRACE = "CLAUDE Layer 5 chunk-1 disposition features/regime_mvp_context.py 1-62"
PATH = "features/regime_mvp_context.py"
LO, HI = 1, 62
ANCHOR = (
    "line anchored HEAD b9fd10b; Action 12.10 mvp_vwap_side fail-closed; "
    "FIND-MVP2 mvp_net_gamma coerce; FIND-MVP1 mvp_zone unknown sentinel disclosed"
)
...
FORMAL_KEEP_DERIVED: list[tuple[int, str, str]] = [
    (19, "require_mvp_features fail-closed dict gate", zero("require_mvp_features")),
    (26, "mvp_zone canonical structure.zone read", zero("mvp_zone")),
    (34, "mvp_spot float-or-None price.spot", zero("mvp_spot")),
    (46, "mvp_vwap_side fail-closed anchor.vwap_side", zero("mvp_vwap_side")),
    (54, "mvp_nearest_distances_for_regime", zero("mvp_nearest_distances_for_regime")),
    (61, "mvp_net_gamma float-or-None structure.net_gamma", zero("mvp_net_gamma")),
]
...
def main() -> None:
    ...
    for line in range(LO, HI + 1):
        ...
        row = _synth_row(
            ...
            "NOT_MARKET_DATA",
            notes="MVP canonical feature reads (InferenceSnapshotV1 features dict)",
        )
        col += 1
        out_by_id[row["register_id"]] = row
