...
def _staged_text() -> str:
    ...
    RC-506. This returned `p.stdout or ""` and never read returncode, so any failure of the
    child — a broken or locked index, a corrupt object, git absent from PATH under a
    different launch context — yielded an empty diff. An empty diff scans clean, so this
    BLOCKING pre-commit secrets gate would print its success line and let the commit through
    without having read a single byte of what was being committed. The gate's whole subject
    is "what is about to enter history", and "I could not look" is not "there is nothing".
    """
    p = subprocess.run(
...
def main() -> int:
    try:
        ...
        # RC-506: fail CLOSED. Reporting "clean" for a diff nobody read is the one outcome a
