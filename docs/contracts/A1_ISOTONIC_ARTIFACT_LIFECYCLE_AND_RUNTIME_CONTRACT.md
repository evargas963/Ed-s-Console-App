...
A1 conformal promotion uses **only** the v2 isotonic artifact path defined here. `ml_predict`'s calibrated probability does **not** satisfy precondition 7 or 8 of `docs/contracts/A1_CONFORMAL_INTERVAL_PROMOTION_CONTRACT.md`. The two paths produce different numerical calibrated probabilities — they are not lineage-equivalent.

`a1_ml_predict_to_v2_calibration_bridge_pending` (named in `docs/contracts/A1_CALIBRATED_PROBABILITY_PROVENANCE_CONTRACT.md`) **remains open** until equivalence is empirically validated OR runtime is fully rewired through this v2 path. This contract does not draft a bridge; doing so without empirical equivalence would risk a paper waiver for model mismatch.
...
- `a1_ml_predict_to_v2_calibration_bridge_pending` - referenced from `docs/contracts/A1_CALIBRATED_PROBABILITY_PROVENANCE_CONTRACT.md`; stays open.
