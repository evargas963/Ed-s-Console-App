...
- lineage match failure modes are tested per `docs/contracts/A1_CALIBRATED_PROBABILITY_PROVENANCE_CONTRACT.md` test bar (match, mismatch, missing marker, empty marker), with explicit `precondition_8_calibration_lineage_match_failed` status assertion;
...
- `docs/contracts/A1_CALIBRATED_PROBABILITY_PROVENANCE_CONTRACT.md` (2B) - defines canonical calibration source, runtime marker shape, lineage match comparison rule, and the failure status emitted when precondition 8 fires.
