...
| 17 | DB counts + crosses (8154) | snapshot counters | delegates: `db.py` | **EXTRACTED** — `_db_counts_and_crosses_for_state(ticker, ed_db, walls) -> _DbCountsAndCrossesForState` (4-field `NamedTuple`: db_counts, ceil_tests, floor_tests, recent_crosses). `ed_db` is a parameter (it is `_fetch_state`'s own local `_ed_db = get_db() if _HAS_SIGNALS else None`, not a module global). A missing `ed_db` or a DB query exception both leave every output at its pre-initialized "no data" default. Thirteenth slice; see `tests/test_fetch_state_db_counts_crosses_phase_v1.py`. The original "DB counts + crosses" banner also covered order-flow-data assembly and candle-volume resolution (both still inline, immediately after this extracted block, before the "Build MarketState" banner) — those are separate, not-yet-extracted sub-phases of the same original banner. |
