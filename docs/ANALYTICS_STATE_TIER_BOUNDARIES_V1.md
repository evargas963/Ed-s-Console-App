...
| 4 | Exposures (7284) | GEX/DEX/vanna by strike | delegates: `math_exposure_core.compute_exposures_by_strike` | **EXTRACTED** — `_exposures_for_state(ticker, client, spot, contracts_use, parsed_quote_time, parsed_trade_time, total_vol, log_only, chain_priority) -> _ExposuresForState` (13-field `NamedTuple`: spot_f, tick_ts, bar counts, exposures, diag, cons/gamma strikes, institutional pin, rows/walls/totals, and `client`). The `if spot is None: return ...` guard stays inline in `_fetch_state` (a helper cannot early-return the caller); this phase assumes a real spot. Includes the candle tick-feed + seed-from-price-history-on-stale-grid sub-step, which independently calls Schwab's price-history transport (`safe_get_price_history`) — required a new `governance/provenance_rows.py` row (`server.py:_exposures_for_state`) mirroring `_fetch_state`'s own transport-reaching row, since the call site moved out of `_fetch_state`'s own body. `client` is returned (not just consumed) because the seed path's `client = get_client()` rebinds it — preserved exactly via the return value rather than silently dropped. Ninth slice; see `tests/test_fetch_state_exposures_phase_v1.py`. |
| 5 | Gamma flip + void zones (7387) | wall/flip selection | delegates: `math_levels.py` | **EXTRACTED** — `_gamma_flip_and_void_zones_for_state(ticker, contracts_use, spot_f, exposures, totals, rows) -> _GammaFlipAndVoidZonesForState` (7-field `NamedTuple`). Also feeds the module-level `_iv_tracker` singleton (ATM IV tick + direction read) and derives `consensus_summary` from the Exposures phase's own `rows`. No try/except in the original inline code — an exception here still propagates out of `_fetch_state` unchanged. Tenth slice; see `tests/test_fetch_state_gamma_flip_void_zones_phase_v1.py`. |
| 6 | Charm (7406) | dealer charm by strike | delegates: `math_levels.compute_charm_by_strike` | **EXTRACTED** — `_charm_for_state(ticker, contracts_use, spot_f, selected_exp) -> _CharmForState` (5-field `NamedTuple`: charm_net/charm_dir/charm_toward/charm_mag/charm_drivers), extracted verbatim from `math_exposure.compute_net_charm`. All five outputs pre-initialized to their "unavailable" defaults before the try block; the whole computation is wrapped in try/except Exception that logs and swallows — never raises out of `_fetch_state`. Eleventh slice; see `tests/test_fetch_state_charm_phase_v1.py`. |
...
| 15 | Vol envelope, level density, sector strength (7980) | breadth/regime context | delegates: several `*_engine.py` modules | **EXTRACTED** — `_vol_envelope_and_sector_for_state(ticker, spot_f, atr, walls, mkt_ctx, cache_key) -> _VolEnvelopeAndSectorForState` (7-field `NamedTuple`: vol_ctx, vol_envelope, level_density, sector_strength, index_strength, spy_strength, iwm_deep). Preserves the phase's single most important quirk: `vol_ctx`'s own construction (VIX tracker tick, `MarketVolContextV1`, `record_market_vol_observation`) sits OUTSIDE the try/except wrapping envelope/density/sector-strength/iwm-deep — a swallowed exception there must degrade only those fields, never leave `vol_ctx` unbound (which would break the whole serve cycle downstream). Sixteenth slice; see `tests/test_fetch_state_vol_envelope_sector_phase_v1.py`. |
| 16 | Zone tracking (8120) | price zone state machine | inline + `market_state.py` | **EXTRACTED** — `_zone_tracking_for_state(ticker, consensus_summary) -> dict` (returns the per-ticker zone-tracker dict). Reads/writes the module-level `_zone_tracker` singleton directly. Twelfth slice; see `tests/test_fetch_state_zone_tracking_phase_v1.py`. |
| 17 | DB counts + crosses (8154) | snapshot counters | delegates: `db.py` | **EXTRACTED** — `_db_counts_and_crosses_for_state(ticker, ed_db, walls) -> _DbCountsAndCrossesForState` (4-field `NamedTuple`: db_counts, ceil_tests, floor_tests, recent_crosses). `ed_db` is a parameter (it is `_fetch_state`'s own local `_ed_db = get_db() if _HAS_SIGNALS else None`, not a module global). A missing `ed_db` or a DB query exception both leave every output at its pre-initialized "no data" default. Thirteenth slice; see `tests/test_fetch_state_db_counts_crosses_phase_v1.py`. The original "DB counts + crosses" banner also covered order-flow-data assembly (now its own fourteenth slice, `_order_flow_data_for_state`, see `tests/test_fetch_state_order_flow_data_phase_v1.py`) and candle-volume resolution (now its own fifteenth slice, `_candle_volume_for_state(ticker, client) -> Optional[float]`, see `tests/test_fetch_state_candle_volume_phase_v1.py` — preserves a pre-existing quirk where the banner's own comment claims "price history primary, accumulator secondary" but the real order tries accumulator first, price history second, then re-checks the accumulator a third time with an identical, unreachable condition; also calls `safe_get_price_history` again, requiring a new `governance/provenance_rows.py` row mirroring `_fetch_state`'s own transport-reaching row). Removed two now-dead lines from `_fetch_state`'s own body as a direct, mechanical consequence of this extraction: `_c_vol = None` and `_completed_for_vol = _candles_1m.get_bars(ticker)` had zero remaining readers once the candle-volume computation moved into its own function (which recomputes its own local copy) — keeping them would have introduced a new F841 ruff finding, breaking the established organic-drift baseline. |
| 18 | Build `MarketState` dataclass (8328) | assembles the typed observation | delegates: `market_state.build_market_state` | **Deliberately NOT extracted** — evaluated and skipped. Unlike every other phase, this block has no computation of its own: it is a single call to the already-separate `market_state.build_market_state` with ~40 keyword arguments, each threading an earlier phase's own output, plus a `try/except: _diag_crash(...); raise` (re-raise, not swallow). Wrapping this in its own function would only relocate a large keyword-argument list — the same marshaling complexity, one layer further away — without creating a genuine testable unit of logic the way every previously extracted phase did. Left inline. |
| 18b | Section 8 (post-build) — Sweep Score (8853) | sweep score from wall proximity + void factor + momentum | delegates: `math_probabilities.compute_sweep_score` | **EXTRACTED** — `_post_build_sweep_score_for_state(ms, atr, candle_body, void_factor) -> dict`. Must run strictly after `build_market_state` — taking `ms` as an explicit parameter makes this structurally enforced (not just textually positioned), since Python cannot bind an argument to a name that doesn't yet exist. This is the FIND-SERVER-SWEEP-DEAD-FEED fix's own governing shape (`tests/test_server_sweep_score_post_build_market_state.py`, updated to check the extracted function's own source). Seventeenth slice; see `tests/test_fetch_state_post_build_sweep_score_phase_v1.py`. |
| 18c | Additive context — liquidity behavior + news/sentiment (non-authoritative) | `institutional_behavior.compute_liquidity_behavior_row` + `news_sentiment.refresh_and_context_for_ui` | delegates fully | **EXTRACTED** — `_additive_context_for_state(ms, ticker, ed_db, spot_f, c_open, c_high, c_low, c_close, c_vol, flow_imb_norm, atr, c_range, candle_body) -> None`. Mutates `ms.liquidity_behavior`/`ms.news_context` in place, no return value — matches the original inline code's own side-effect-only shape. Two fully independent try/except blocks; a liquidity-behavior failure never affects news_context and vice versa. Eighteenth slice; see `tests/test_fetch_state_additive_context_phase_v1.py`. |
