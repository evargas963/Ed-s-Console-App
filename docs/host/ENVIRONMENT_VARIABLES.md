...
| Situation | Contract |
|-----------|----------|
| Empty or schema-less explicit test DB / canonical `data/ed_console.db` | **In-scope** for `--objective-audit` and governance pytest — `db.ensure_console_db_training_schema()` bootstraps required tables before audit reads |
