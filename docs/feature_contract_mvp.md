...
| `liquidity.range_imbalance_stall_score` | `int` / `float` | yes | N/A | rejected | [0, 1] stall × |imbalance|; never a legacy absorption_score |
| `liquidity.range_imbalance_push_score` | `int` / `float` | yes | N/A | rejected | [0, 1] push × |imbalance|; never a legacy continuation_score |
