...
**Agent operating process (operator 2026-08-03, RC-217; charter `governance/AGENT_OPERATING_PROCESS_V1.md`; enforced by `tools/operating_process_lock.py`, `tools/process_lock_guard.py`, pre-commit `operating-process`).** Mandatory checklist for Claude + Cursor: sole writer (`governance/sole_writer.json`), MEASURE-before-claim (index/HEAD/WT hashes), small landings, long-timeout pre-commit (never kill mid-hook), LIVE vs DISK (`DISK_ONLY_UNTIL_RESTART` until restart proven), auditor window, operator GO for held staged surfaces (`governance/operator_go.json`). PreToolUse BLOCKs dual-edit on protected paths; Stop BLOCKs COMPLETE/LIVE/parity claims while index≠WT or live PID predates `db.py` gate; commit BLOCKs without parity or GO.

**Cursor Project Manager (operator 2026-08-03, RC-218/RC-219/RC-220; `governance/REHAB_PROGRAM.md`, `governance/PM_MANDATE.md`, `.cursor/rules/07-cursor-pm.mdc`, `tools/rehab_daily_scan.py`, `governance/pm_mission.json`).** Cursor is standing PM for **whole-repo rehab** (piece-by-piece, end-to-end, no patches) — not a single-feature valet. Default program is rehab facets in REHAB_PROGRAM; named missions are slices. **Change requests:** operator → Cursor plan → GO → `pm_mission.json` → writer → audit. Product edits without an active mission BLOCK. Daily rehab Automation/scan is **recommend-only**. PM must keep the operator straight (Automation, GO, restart) without being reminded. `sole_writer.json` must keep `"pm": "cursor"`.

**Writer no-drift law (operator 2026-08-03, RC-226; enforced by `check_writer_no_drift`, `tools/writer_drift_lock.py`, `tools/process_lock_guard.py` / `pm_mission_edit_violation`).** When an in-progress mission assigns `writer` ≠ current agent, the non-writer must not modify mission `scope_paths` (Cursor must not absorb Claude's sole-writer work; mirror when writer=cursor). PM/auditor allowlist only. Deny prefix `SOD_DRIFT:`. Staged scope by the wrong role → commit BLOCK. Self-heal on drift without operator prompt. Escape: `ED_WRITER_DRIFT_GUARD=off`.

**RC document-without-resolve law (operator 2026-08-03, RC-228; enforced by `check_rc_document_without_resolve`, `tools/rc_resolve_lock.py`, `completion_claim_violations` in `tools/operating_process_lock.py`).** Writers must not grow an OPEN/PARTIAL documentation backlog without resolve. Newly ADDED OPEN RC rows require `FIXED:` / `NEXT-DEPTH:` / `OUT-OF-SCOPE:` in the fix cell. Staging `pm_mission.json` to DONE/COMPLETE/idle while any OPEN RC names that `mission_id` BLOCKs. PM rejects mission COMPLETE while those OPEN RCs remain. Honest PARTIAL + tracker is legal; mass-fake CLOSE is not. Escape: `# mission-rc-open-ok:` / `ED_RC_RESOLVE_GUARD=off`. Drain triage: `reports/rc_open_drain_latest.md`.
