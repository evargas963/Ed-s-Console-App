...
**Find something broken → fix it.** Discovery creates the obligation to remediate through the full blast radius in the active session. A material defect is never disposed as queued / logged / TODO / follow-up / pre-existing / out-of-scope; if it genuinely cannot be fixed now, say exactly what blocks it, in plain sight. When implementation work exposes a material defect in the path being changed: inspect enough, establish the cause sufficiently, FIX, prove it, continue — do not spawn a separate audit mission when the cause is already sufficient to repair safely. Discovery ENDS once four things are established: the material defect, its lowest proven root cause, its canonical owner, and the connected repair path. From that point the only normal sequence is FIX → PROVE THE FIX → CLOSE / CONTINUE. If the proof fails, return to FIXING that failed repair, then re-prove; do not return to discovery and do not commission another audit unless new evidence materially changes the root cause, the canonical owner, or the required connected scope. A new manifestation, an edge case, a failed test, or a failed repair does not by itself reopen discovery — it is the next fix in the same mission. Mechanism: `tools/mission_latch.py` — one work row open before a production mutation, and a turn may not end while that row is unfinished and unblocked.
