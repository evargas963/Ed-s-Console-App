...
**Cursor Project Manager (operator 2026-08-03, RC-218/RC-219; `governance/PM_MANDATE.md`, `.cursor/rules/07-cursor-pm.mdc`, `tools/rehab_daily_scan.py`, `governance/pm_mission.json`).** Cursor is standing PM: one mission, stop thrash, GO/NO-GO, triage `reports/rehab_latest.md`. **Change requests:** operator → Cursor plan → operator GO → `pm_mission.json` active → writer → Cursor audit. Product-path edits without an active mission are BLOCKED (`pm_mission_edit_violation` / `process_lock_guard`). Daily rehab is **recommend-only**. `sole_writer.json` must keep `"pm": "cursor"`.
