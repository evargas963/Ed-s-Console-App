...
Everything mechanical lives in three places and nowhere else: the guard chains in
`.claude/settings.json` / `.cursor/hooks.json` (in session), `.pre-commit-config.yaml` (at
commit), and required CI — `pytest-full` + `hardening`, which run
...
**Close contract.** A `CLOSED` root-cause row carries a causally sufficient why-chain (it ends at the wrong model, assumption, semantics, placement or computation, each link a measured fact), at least one backticked re-runnable command as its evidence, and, where it says a code change exists, the named files staged with the row or carried by a cited commit. Enforced by `check_root_cause_log` for the checkable parts (schema, dates, status vocabulary, the command, the carrying commit, overdue rows); causal sufficiency is judged in review, never by counting. **Recurrence proof covers the seam, not the incident's path.** Where the seam the defect lives on publishes a canonical enumeration of its own entries, the recurrence control drives EVERY registered entry, derived from that enumeration at the seam's owner, never only the entry the incident happened through (RC-531: the rule was installed in the Stop entry while the PreToolUse entry reached the same executor and still trusted argv; its controls, CI and merge all passed). Today the hook seam publishes one: the wiring in `.claude/settings.json`, read by `tools.stop_chain.registered_entrypoints`; a seam that later publishes its own enumeration is held to the same rule by that owner. No hand-maintained list of entries, in a test or anywhere else, is a second source of truth. Where a seam has no authoritative enumeration, materially connected scope is judged in review and dual signoff, never claimed by a machine.
