...
**Five Whys (operator 2026-08-13):** when an issue is encountered — test fail, audit finding, operator correction, stale claim vs HEAD, false close, parked PR, repeated class — run five recursive Whys in chat before the next edit of that class. Why-1 is the symptom. Why-5 must be a mechanism (missing predicate, stale evidence used as spec, completion defined as an intermediate artifact), not a person ("I forgot"). Then name the prevention: a rule in this file if the class is new, and a gate/test if a machine can see it. Do not ship the symptom-fix and skip bedrock. Do not open a new program from a missing import. Immune: this protects same-error recurrence; the page, the pre-work question, and green CI cannot see why the last miss happened.

**Named force functions (operator 2026-08-14):** chat is not a lock. If this agent names a force / bind / lock, it lands in this file and in a test that fails if the sentence disappears, in the same program — or the same sentence must say "conduct only." Slice locks (every change): (1) smallest complete change; tests lock the claim; (2) matching `tools/check_*.py` if one applies + smallest pytest that hits the path, output shown; (3) drift-audit skill before any done / MET / `[x]` / "on main" claim; (4) CI green, then merge; finish = `git merge-base --is-ancestor <sha> origin/main`; (5) Five Whys on any issue before repeating the class; (6) Bugbot when the diff is material to Collect / Find & Prove / Decide (producer, faucet, identity, decision path, board closure); (7) security-review when secrets, serve, or a faucet moved. (1)(2)(4) are mechanical where CI/pre-commit already run. (3)(5)(6)(7) are charter-mandatory conduct — a machine cannot see whether the agent ran them; the presence lock only proves these sentences still exist. Immune: this protects "promised bind, not in the repo."
